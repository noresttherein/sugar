package net.noresttherein.slang.concurrent

import java.lang.invoke.MethodHandles

import net.noresttherein.slang.concurrent.SpinLock.{ownerField, Open, SpinsBetweenChecks}
import net.noresttherein.slang.exceptions.{AbstractThrowable, StackableException}
import net.noresttherein.slang.time.{Milliseconds, PosixTime}
...
/** A simple spin lock implementation based around a `@volatile` flag.
  ...
  * locking of the Lock with one of [[net.noresttherein.slang.concurrent.SpinLock.lock lock]] methods;
  * leaving it involves unlocking with [[net.noresttherein.slang.concurrent.SpinLock.unlock unlock]]
  * or [[net.noresttherein.slang.concurrent.SpinLock.close close]]. Once locked, all threads attempting to lock it
  ...
  * by other threads using [[net.noresttherein.slang.concurrent.SpinLock.forceOpen forceOpen]] instead of `unlock`.
  *
  * Additionally, any thread at any time can [[net.noresttherein.slang.concurrent.SpinLock.break break]] the lock:
  * this will cause all future attempts to lock or unlock it to fail
  * with a [[net.noresttherein.slang.concurrent.BrokenLockException BrokenLockException]], and any threads
...
//	def isOpen :Boolean = owner == 0L
	...
	/** Checks if this lock is owned by the currently executed thread. In general, once the thread acquires
	  ...
	  * [[net.noresttherein.slang.concurrent.SpinLock.forceOpen forceOpen]] which evicts any thread from the lock
	...
	/** Attempts to lock this lock, burning CPU cycles until some other thread releases it (unless currently open).
	  * If the lock is, or becomes [[net.noresttherein.slang.concurrent.SpinLock.break broken]], the method
	  ...
	  * [[net.noresttherein.slang.concurrent.SpinLock.unlock unlock]]`()` as soon as possible.
	  */
	...
	/** Attempts to lock this lock, burning CPU cycles until some other thread releases it (unless currently open).
	  * It is similar to parameterless [[net.noresttherein.slang.concurrent.SpinLock.lock lock()]]`()`,
	...
	/** Opens the lock. Only the thread which currently holds the lock can unlock it.
	  * @see [[net.noresttherein.slang.concurrent.SpinLock.close]]
	  */
	...
	/** An [[AutoCloseable]] hookup which releases the lock. Rather unfortunately named, as it actually
	  * ''opens'' (unlocks) the lock. Similar to [[net.noresttherein.slang.concurrent.SpinLock.unlock unlock]]`()`,
	...
	/** Breaks the lock, entering a state in which can no longer be locked, but neither is open.
	  ...
	  * by throwing a [[net.noresttherein.slang.concurrent.BrokenLockException BrokenLockException]].
	  * All calls to [[net.noresttherein.slang.concurrent.SpinLock.unlock unlock]]
	  * or [[net.noresttherein.slang.concurrent.SpinLock.forceOpen forceOpen]] will also end with the same exception
	  * being thrown.
	  */
...
/** An exception thrown when a [[net.noresttherein.slang.concurrent.SpinLock SpinLock]]
  * becomes [[net.noresttherein.slang.concurrent.SpinLock.break broken]].
  */
