...
import net.noresttherein.slang.collection.MutableEqSet
...
package object exceptions extends exceptions.imports with exceptions.extensions {
	...
	private def causeCycleException(e :Throwable) =
		new SlangException(s"Cycle detected in the exception stack: ${e.getCause} already present.")
...
//						causeTrace.drop(i)
	...
	/** Prints stack trace using `printer` argument.
	  * Used by [[net.noresttherein.slang.exceptions.Rethrowable Rethrowable]] as this method
	  * doesn't attempt to use the internal initialization of the stack trace, but rather uses the public
	  * [[net.noresttherein.slang.exceptions.AbstractThrowable AbstractThrowable]]`.`[[net.noresttherein.slang.exceptions.AbstractThrowable.stackTrace stackTrace]]
