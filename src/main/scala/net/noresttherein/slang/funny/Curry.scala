...
object Curry {
	...
	sealed abstract class ReturnType[F, X, Y] {
		...
		/** As this instance witnesses that `F &lt;: Mapped[X=>Y]`, convert the given function so that it conforms to the return type.
	...
	/** Base trait for partially applied functions implemented as [[net.noresttherein.slang.funny.Curry.Curried!]].
	  ...
	  */
	sealed abstract class PartiallyApplied[X, Y] {
	...
	final class Curried[A[+R], X, Y](val unapplied :A[X=>Y])(val curry :Curry[A, X, Y]) extends PartiallyApplied[X, Y] {
		...
		/** Map the result of partial application of this function up to argument X. Differs from [[mapped]] in that
		  ...
		  */
		@inline def returning[O](map :Y=>O) :Mapped[X=>O] =
