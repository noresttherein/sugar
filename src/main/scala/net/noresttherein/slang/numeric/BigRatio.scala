...
import net.noresttherein.slang.numeric.BigRatio.GCD
...
/** Simple implementation of rational numbers (fractions of integer values), represented by a pair of `BigInt` values.
  ...
  * see [[net.noresttherein.slang.numeric.BigRatio.apply(numerator:Int, denominator:Int)* BigRatio()]].
  * There is also an extension method [[net.noresttherein.slang.numeric.BigRatio.bigRatio_%/ bigRatio_%/]]
  * (available also as `slang.`[[net.noresttherein.slang.numeric.extensions.method_%/(self:BigInt) method_%/]])
                              ...
                              private[this] var num :BigInt, private[this] var den :BigInt) extends Number
{
	...
	/** `-this.abs`. This method exists only for consistency with
	  *  [[net.noresttherein.slang.numeric.Ratio Ratio]]`.`[[net.noresttherein.slang.numeric.Ratio.minusAbs minusAbs]]:
	  *  `BigInt` does not overflow and there is no particular need for using `minusAbs` instead of
	  *  [[net.noresttherein.slang.numeric.BigRatio.abs abs]].
	  */
...
/** Companion object for the [[net.noresttherein.slang.numeric.BigRatio! BigRatio]] class representing rational numbers
...
//			case -100L if numerator.toInt == numerator => -percent(-numerator.toInt)
	...
	/** Introduces method [[net.noresttherein.slang.numeric.BigRatio.BigIntNumerator.%/ %/]] extension method to `Int`,
	  * allowing to create a [[net.noresttherein.slang.numeric.BigRatio BigRatio]] through a natural looking expression
	...
	/** A builder of [[net.noresttherein.slang.numeric.BigRatio BigRatio]] objects, accepting an `Int` denominator
	...
	class BigIntNumerator(private val numerator :BigInt) extends AnyVal {
		...
		/** Divides this `BigInt` by the argument, creating a [[net.noresttherein.slang.numeric.BigRatio BigRatio]]
