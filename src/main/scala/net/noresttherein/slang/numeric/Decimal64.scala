...
import net.noresttherein.slang.exceptions.InternalException
import net.noresttherein.slang.numeric.Decimal64.{divideByDigits, divideLong, roundDivision, throwArithmeticException, trailingZeros, Decimal64AsIfIntegral, DoublePowersOf10, ExtendedPrecision, FloatPowersOf10, LongPowerBounds, LongPowersOf10, LongPrecision, MaxDigitsInPlainString, MaxDigitsInWholeString, MaxExponent, MaxFractionalDigitsInPlainString, MaxLeadingZerosInString, MaxLongPowerOf10, MaxLongPrecision, MaxLongValue, MaxLongValueLog, MaxPrecision, MaxUnscaled, MaxWholeDigitsInPlainString, MinLongValue, MinScale, MinUnscaled, MinusOne, NegativeExponentFormat, One, PositiveExponentFormat, PowersOf10, Precision, PrecisionExceededException, Round, RoundingNecessaryException, ScaleBits, ScaleMask, ScaleSign, SignificandBits, SignificandMask, Zero}
import net.noresttherein.slang.numeric.Decimal64.implicits.{IntScientificDecimal64Notation, LongScientificDecimal64Notation}
import net.noresttherein.slang.numeric.Decimal64.Round.{isNearestNeighbour, to16digits, to17digits, toMaxDigits, Extended, ExtendedExact, ExtendedHalfEven}
import net.noresttherein.slang.oops
import net.noresttherein.slang.vars.Opt
import net.noresttherein.slang.vars.Opt.Got
import net.noresttherein.slang.witness.Maybe
...
/** A value class implementing rational numbers with a finite decimal expansion. Each value is represented as
  * [[net.noresttherein.slang.numeric.Decimal64.unscaled unscaled]]` * 10^-`[[net.noresttherein.slang.numeric.Decimal64.scale scale]],
  ...
  * [[net.noresttherein.slang.numeric.Decimal64$ Decimal64]]`.`[[net.noresttherein.slang.numeric.Decimal64.Standard Standard]]).
  ...
  *      and [[net.noresttherein.slang.numeric.Decimal64.MinScale minimum scale]] (maximum exponent) fitting
  ...
  *      [[net.noresttherein.slang.numeric.Decimal64.significand significand]]`*10^`[[net.noresttherein.slang.numeric.Decimal64.exponent exponent]].
  *      To compensate, it introduces properties [[net.noresttherein.slang.numeric.Decimal64.precision precision]],
  *      [[net.noresttherein.slang.numeric.Decimal64.fractionalDigits fractionalDigits]] and
  *      [[net.noresttherein.slang.numeric.Decimal64.wholeDigits wholeDigits]].
  *   1. Like `java.math.BigDecimal`, but unlike `scala.math.BigDecimal`, it has no inherent associated
  *      [[java.math.MathContext MathContext]], which instead can be provided to its methods an argument.
  ...
  *      [[net.noresttherein.slang.numeric.Decimal64$ Decimal64]]`.`[[net.noresttherein.slang.numeric.Decimal64.Round Round]]`.`[[net.noresttherein.slang.numeric.Decimal64.Round.Exact Exact]]
  *      or [[net.noresttherein.slang.numeric.Decimal64$ Decimal64]]`.`[[net.noresttherein.slang.numeric.Decimal64.Round Round]]`.`[[net.noresttherein.slang.numeric.Decimal64.Round.ExtendedExact ExtendedExact]]
  *      mode instead. The 'extended' rounding modes defined in
  *      [[net.noresttherein.slang.numeric.Decimal64$ Decimal64]]`.`[[net.noresttherein.slang.numeric.Decimal64.Round Round]]
  *      specify this behaviour, with [[net.noresttherein.slang.numeric.Decimal64.Round.Extended Extended]]
  *      (equal to [[net.noresttherein.slang.numeric.Decimal64.Round.ExtendedHalfEven ExtendedHalfEven]])
  ...
  *      than their official counterparts, mostly due to the difference from the above point.
	...
	extends AnyVal with ScalaNumericAnyConversions with Serializable
{
	...
	/** The value of this `Decimal64` with the decimal fraction point shift right
	  * by [[net.noresttherein.slang.numeric.Decimal64.scale scale]].
	  * It is `x` in the `x * e^10` underlying representation of this value.
	  * If this decimal contains no more than `-`[[net.noresttherein.slang.numeric.Decimal64.MinScale MinScale]] zeros,
	  ...
	  * `[`[[net.noresttherein.slang.numeric.Decimal64.MinUnscaled MinUnscaled]]..[[net.noresttherein.slang.numeric.Decimal64.MaxUnscaled MaxUnscaled]]`]`).
	  * @see [[net.noresttherein.slang.numeric.Decimal64.scale scale]]
	  * @see [[net.noresttherein.slang.numeric.Decimal64.significand coefficient]]
	  **/
	...
	/** The scale of this `Decimal64`, that is the number of fractional digits in the decimal representation
	  ...
	  * [[net.noresttherein.slang.numeric.Decimal64.unscaled unscaled]] in the decimal representation
	  ...
	  * (`[`[[net.noresttherein.slang.numeric.Decimal64.MinScale MinScale]]..[[net.noresttherein.slang.numeric.Decimal64.MaxScale MaxScale]]`]`).
	...
	/** The [[net.noresttherein.slang.numeric.Decimal64.unscaled unscaled]] value of this decimal divided
	  * by its maximal power of ten divisor.
	  * @see [[net.noresttherein.slang.numeric.Decimal64.exponent exponent]]
	  **/
	...
	/** The number of trailing zeros in the decimal representation of this number (for whole numbers),
	  ...
	  * Note that this is not the same as `-`[[net.noresttherein.slang.numeric.Decimal64.scale scale]],
	  * as the latter does not take into account
	  * that [[net.noresttherein.slang.numeric.Decimal64.unscaled unscaled]] may have trailing zeros.
	...
	/** Number of digits in [[net.noresttherein.slang.numeric.Decimal64.unscaled unscaled]].
	...
	/** The number of decimal digits in [[net.noresttherein.slang.numeric.Decimal64.significand significand]],
	  * that is the number of digits in this decimal, not counting trailing zeros (on whole numbers).
	  * Note that this is different from [[net.noresttherein.slang.numeric.Decimal64.unscaledDigits unscaledDigits]],
	  * in that the latter counts trailing zeros in [[net.noresttherein.slang.numeric.Decimal64.unscaled unscaled]].
	  * it complements well the [[net.noresttherein.slang.numeric.Decimal64.fractionalDigits fractionalDigits]] property,
	...
	/** The total number of decimal digits in this value, both fractional and whole. Returns `1` if `-1 < this < 1`.
	  ...
	  * @see [[net.noresttherein.slang.numeric.Decimal64.wholeDigits]]
	  * @see [[net.noresttherein.slang.numeric.Decimal64.fractionalDigits]]
	  **/
	...
	/** The number of decimal digits before the fractional point.
	  ...
	  * @see [[net.noresttherein.slang.numeric.Decimal64.digits]]
	  * @see [[net.noresttherein.slang.numeric.Decimal64.fractionalDigits]]
	  **/
	...
	/** Number of fractional digits in this value's decimal representation with no trailing zeros.
	  * @see [[net.noresttherein.slang.numeric.Decimal64.precision]]
	  * @see [[net.noresttherein.slang.numeric.Decimal64.exponent]]
	...
	/** The absolute value of this number.
	  * @see [[net.noresttherein.slang.numeric.Decimal64.minusAbs]]
	  **/
	...
	/** Overflow safe `-this.abs`. Due to details of binary encoding, just as with inbuilt value types,
	  * `Decimal64` with a significand of [[net.noresttherein.slang.numeric.Decimal64.MinUnscaled MinUnscaled]]
	  * does not have a positive counterpart. For these values, [[net.noresttherein.slang.numeric.Decimal64.abs abs]]
	...
	/** Moves the decimal dot right by the number of digits in the fractional part of this decimal, if non-negative.
	  ...
	  * Together with [[net.noresttherein.slang.numeric.Decimal64.denominator denominator]] it presents
	  ...
	  *
	  * This method does not throw any exceptions.
	  * @see [[net.noresttherein.slang.numeric.Decimal64.toRatio]]
	  * @see [[net.noresttherein.slang.numeric.Decimal64.toIntRatio]]
	  **/
	...
	/** If this decimal number has `n >= 0` digits in its fractional part, return `10^n`; otherwise return `1`.
	  ...
	  * Together with [[net.noresttherein.slang.numeric.Decimal64.numerator numerator]] it presents a decimal fraction
	...
	/** Calculates `1 / this` to the precision defined by an implicit [[java.math.MathContext MathContext]]
	  * (defaults to [[net.noresttherein.slang.numeric.Decimal64.Round.Extended Extended]], which is the maximum
	...
	/** True if this number's [[net.noresttherein.slang.numeric.Decimal64.whole whole]] part is zero.
	...
	/** The fractional part o this decimal number. For negative values, it is equal to `this.abs.fraction`.
	  * This method does not throw any exceptions.
	  * @return a number equal to `this - `[[net.noresttherein.slang.numeric.Decimal64.whole whole]].
	  **/
	...
	/** This decimal number rounded towards lesser absolute value.
	  * For negative values, this method is equivalent to [[net.noresttherein.slang.numeric.Decimal64.ceiling ceiling]];
	  * for non-negative, the result equals [[net.noresttherein.slang.numeric.Decimal64.floor floor]].
	  * This method does not throw any exceptions.
	  * @return `this.`[[net.noresttherein.slang.numeric.Decimal64.whole whole]].
	  **/
	@throws[ArithmeticException]("if underflow occurs.")
	def roundDown :Decimal64 = whole //round(0, DOWN)

	/** This decimal number rounded towards greater absolute value.
	  * For negative values, this method is equivalent to [[net.noresttherein.slang.numeric.Decimal64.floor floor]];
	  * for non-negative, the result equals [[net.noresttherein.slang.numeric.Decimal64.ceiling ceiling]].
	  * @return [[net.noresttherein.slang.numeric.Decimal64.roundDivision(fractional:Int,rounding:RoundingMode* round]]`(0, UP)`.
	  **/
	@throws[ArithmeticException]("if overflow occurs.")
...
//				Decimal64.round(m, 0, rounding, precision - s + fractional)
	...
	/** Rounds this number to `mode.`[[java.math.MathContext.getPrecision getPrecision]] number of leading digits,
	  ...
	  * [[net.noresttherein.slang.numeric.Decimal64.Round.Extended Extended]] context is used, which rounds
...
*/
	...
	/** Divides `this` by `that`, rounding to the requested precision according to `mode.getRoundingMode` mode.
	  ...
	  * the `[`[[net.noresttherein.slang.numeric.Decimal64.MinUnscaled MinUnscaled]]`, `[[net.noresttherein.slang.numeric.Decimal64.MaxUnscaled MaxUnscaled]]`]`
	  ...
	  * `this.`[[net.noresttherein.slang.numeric.Decimal64.toJavaBigDecimal toJavaBigDecimal]]`.divide(that.toJavaBigDecimal, new MathContext(precision, mode.getRoundingMode))`,
	  * where `precision` equals `mode.getPrecision` if the latter is non zero,
	  * or the [[net.noresttherein.slang.numeric.Decimal64.Round.maxPrecision maximal precision]] with which the result
	...
	/** Performs integral division of `this` by `that`, i.e. returns a maximal in absolute magnitude integral `q`
	  ...
	  *
	  * For values which fall within the precision of `Decimal64`, the result equals
	  * `this`.[[net.noresttherein.slang.numeric.Decimal64.toJavaBigDecimal toJavaBigDecimal]]`.divideToIntegralValue(that.toJavaBigDecimal, MathContext.`[[java.math.MathContext.UNLIMITED UNLIMITED]]`).`[[java.math.BigDecimal.round round]]`(new MathContext(precision, DOWN))`,
	  ...
	  * [[net.noresttherein.slang.numeric.Decimal64.div div]] and other binary arithmetic operations
	  ...
	  * or requesting a precision exceeding [[net.noresttherein.slang.numeric.Decimal64.MaxPrecision MaxPrecision]].
	  **/
	@throws[ArithmeticException]("if that is zero, the result cannot be represented as a Decimal64 with the requested" +
	                             " precision or mode.getRoundingMode is UNNECESSARY and rounding is necessary.")
	...
	/** Raises this number to the given integral power.
	  *   - If `n` is zero, then [[net.noresttherein.slang.numeric.Decimal64.One One]] is returned.
...
//	@inline def ==(that: Long) :Boolean = (this compare that) == 0
	...
	/** Writes the underlying 8 bytes to the stream. The first 7 bytes are the significand, highest byte first,
	  ...
	  * @see [[net.noresttherein.slang.numeric.Decimal64.read]]
	  **/
...
/** A factory of [[net.noresttherein.slang.numeric.Decimal64! Decimal64]] numbers, additionally grouping constants
  ...
  * [[net.noresttherein.slang.numeric.Decimal64.Round Mode]]`.`[[net.noresttherein.slang.numeric.Decimal64.Round.Exact Exact]]
  ...
  * [[net.noresttherein.slang.numeric.Decimal64.Round Mode]]`.`[[net.noresttherein.slang.numeric.Decimal64.Round.Extended Best]]
...
object Decimal64 {
	...
	/** The maximal number such that all values of that many integral digits fit in the
	  ...
	  * @see [[net.noresttherein.slang.numeric.Decimal64.MaxUnscaled]]
	  * @see [[net.noresttherein.slang.numeric.Decimal64.MinUnscaled]]
	  **/
	...
	/** Maximum attainable precision by a `Decimal64` value. Not all values of that many significant digits
	  ...
	  * is less or equal to [[net.noresttherein.slang.numeric.Decimal64.MaxUnscaled MaxUnscaled]].
	  * Equal to [[net.noresttherein.slang.numeric.Decimal64.Precision Precision]]` + 1`.
	  **/
	...
	/** The largest `Decimal64` which can be represented exactly as a `Long`. All instances
	  * [[net.noresttherein.slang.numeric.Decimal64.MinLongValue MinLongValue]]` <= decimal <= MaxLongValue`
	  * are valid `Long` values. There are, however, smaller `Long` values which cannot be represented as a `Decimal64`;
	  * [[net.noresttherein.slang.numeric.Decimal64.MaxUnscaled MaxUnscaled]] is the largest `Long` such that
	  * all lesser values (greater or equal to [[net.noresttherein.slang.numeric.Decimal64.MinUnscaled MinUnscaled]])
	...
	/** The smallest (negative) `Decimal64` which can be represented exactly as a `Long`. All instances
	  * `MinLongValue <= decimal <= `[[net.noresttherein.slang.numeric.Decimal64.MaxLongValue MaxLongValue]]
	  * are valid `Long` values. There are, however, larger `Long` values which cannot be represented as a `Decimal64`;
	  * [[net.noresttherein.slang.numeric.Decimal64.MinUnscaled MinUnscaled]] is the smallest `Long` such that
	  * all greater values (lesser or equal to [[net.noresttherein.slang.numeric.Decimal64.MaxUnscaled MaxUnscaled]])
	  * are representable exactly as `Long` values.
	  **/
	...
	/** [[java.math.MathContext MathContext]] constants with various rounding modes and precision of 16 or maximum digits.
	  ...
	  *      [[net.noresttherein.slang.numeric.Decimal64.Round.Standard Standard]].
	  ...
	  *      [[net.noresttherein.slang.numeric.Decimal64.Round.Extended Extended]] precision itself
	  *      (rounding [[java.math.RoundingMode.HALF_EVEN HALF_EVEN]] is the default mode
	  *      used by [[net.noresttherein.slang.numeric.Decimal64 Decimal64]] class methods when no `MathContext`
	  *      was provided.
	  *   1. [[net.noresttherein.slang.numeric.Decimal64.Round.Exact Exact]] - a single rounding mode demanding
	...
	object Round {
		...
		/** A `Decimal64` representation in extended precision, maximum for a given number in 56 bits
		  ...
		  * It is equal to [[net.noresttherein.slang.numeric.Decimal64.Round.ExtendedHalfEven ExtendedHalfEven]].
		  * It is the mode used by factory methods which do not accept a `MathContext`.
		  * @see [[net.noresttherein.slang.numeric.Decimal64.Round.Standard]] */
		...
		/** A MathContext object with a precision setting matching the IEEE 754R Decimal64 format, 16 digits,
		  ...
		  * `Decimal64.`[[net.noresttherein.slang.numeric.Decimal64.Round.HalfEven HalfEven]].
		  * @see [[net.noresttherein.slang.numeric.Decimal64.Round.Extended]] */
		...
		/** A factory method creating a `MathContext` with a precision of `n` and the given rounding mode.
		  * If the precision is within the `[0, `[[net.noresttherein.slang.numeric.Decimal64.MaxPrecision MaxPrecision]]`]`
...
//	private final val MaxWholeDigitsInString = 4
	...
	/** Attempts to create a `Decimal64` equal to the given `Long` value.
	  ...
	  *            [[net.noresttherein.slang.numeric.Decimal64.MinUnscaled MinUnscaled]]` <= int <= `[[net.noresttherein.slang.numeric.Decimal64.MaxUnscaled MaxUnscaled]].
	...
	/** Creates a normalized `Decimal64` instance equal to `significand * 10E-scale`.
	  ...
	  * [[net.noresttherein.slang.numeric.Decimal64.roundDivision(significand:Long,scale:Int)(implicit*, round]]`(significand, scale)(`[[net.noresttherein.slang.numeric.Decimal64.Round.Extended Best]]`)`
	...
	/** Creates `Decimal64` of value `significand*10^-scale`, rounded to 16 digits in the direction
	  ...
	  * to [[net.noresttherein.slang.numeric.Decimal64.Precision Precision]].
	  **/
	...
	/** Creates `Decimal64` of value `significand*10^-scale`, rounded
	  ...
	  * [[net.noresttherein.slang.numeric.Decimal64.roundDivision(significand:Long,scale:Int)* round]]`(significand, scale)(mode)`,
	...
	/** Parses a `String` with a number in the decimal format and rounds it according
	  ...
	  * as by [[java.math.BigDecimal java.math.BigDecimal]] constructor.
	  * This method is exactly equivalent to
	  * [[net.noresttherein.slang.numeric.Decimal64.apply(string:String,mode:MathContext)* apply]]`(string, mode)`,
...
//	@inline def round(significand :Long, rounding :RoundingMode) :Decimal64 = round(significand, 0, rounding)
	...
	/** Rounds the given integral number according to the rounding mode and to the precision specified
	  ...
	  * [[net.noresttherein.slang.numeric.Decimal64.Round Round]]`.`[[net.noresttherein.slang.numeric.Decimal64.Round.Extended Extended]]
...
//		round(significand, scale, rounding, Precision, false)
	...
	/** Rounds the given [[scala.math.BigDecimal]] according to the rounding mode and to the precision specified
	  ...
	  * [[net.noresttherein.slang.numeric.Decimal64.apply(big:scala.math.BigDecimal,mode:MathContext)* BigDecimal]]`(big, mode)`,
	...
	/** Rounds the given [[java.math.BigDecimal]] according to the rounding mode and to the precision specified
	  ...
	  * [[net.noresttherein.slang.numeric.Decimal64.Round Round]]`.`[[net.noresttherein.slang.numeric.Decimal64.Round.Extended Extended]]
	  * is used instead. This method is equivalent to
	  * [[net.noresttherein.slang.numeric.Decimal64.apply(big:java.math.BigDecimal,mode:MathContext)* BigDecimal]]`(big, mode)`,
	  * the only difference is that the `MathContext` argument is implicit.
	  **/
	@throws[ArithmeticException]("if the value exceeds both the precision of Decimal64 and the one specified by implicit MathContext.")
	...
	/** Parses the given `String` as a decimal and rounds it according the rounding mode and to the precision
	  ...
	  * [[net.noresttherein.slang.numeric.Decimal64.Round Round]]`.`[[net.noresttherein.slang.numeric.Decimal64.Round.Extended Extended]]
	  * is used instead. This method is equivalent to [[net.noresttherein.slang.numeric.Decimal64.parse parse]].
	  **/
	...
	/** Reads the first 64 bits out of the given input stream and interprets them as a `Decimal64`.
	  * The value should have been written with class method [[net.noresttherein.slang.numeric.Decimal64.write write]]
	...
	/** Reinterprets the binary representation of the given `Long` value as a `Decimal64`.
	  ...
	  * `Decimal64.`[[net.noresttherein.slang.numeric.Decimal64.toLongBits toLongBits]], then the created object
	  * may not satisfy the invariants.
	  **/
	@inline private[slang] def fromLongBits(bits :Long) :Decimal64 = new Decimal64(bits)
...
//			case _ if quotPrecision < precision => throw if (sign < 0) UnderflowException else OverflowException
	...
	/** Performs a division `dividend / divisor` within `Long` precision and using
	  * [[net.noresttherein.slang.numeric.Decimal64.roundDivision two-tier rounding]] with `tail` as the second remainder.
