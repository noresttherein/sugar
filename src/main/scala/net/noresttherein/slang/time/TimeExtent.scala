...
/** An umbrella term for specifying length of elapsed time which encompasses both infinite time
  ...
  * For example,
  * [[net.noresttherein.slang.time.Duration Duration]]`(0) == `[[net.noresttherein.slang.time.Milliseconds Milliseconds]]`(0)`,
  * but `!Set(Duration(0))(Milliseconds(0))`.
  * @see [[net.noresttherein.slang.time.InfiniteTimeInterval]]
  * @see [[net.noresttherein.slang.time.TimeFrame]]
  * @see [[net.noresttherein.slang.time.DateInterval]]
  * @see [[net.noresttherein.slang.time.TimeInterval]]
...
sealed trait TimeExtent extends Any with Serializable {
	/** The part of this time extent which is of variable length, that is counted in days and longer units.
	  * For finite time extents this will be a [[net.noresttherein.slang.time.Period Period]], with the exception
	...
	/** The part of this time extent which is of fixed length, that is counted in standard hours and shorter units,
	  ...
	  * and [[net.noresttherein.slang.time.Immediate Immediate]] return themselves,
	  * finite instances will return a [[net.noresttherein.slang.time.Duration Duration]],
	...
	/** This time extent with the variable part normalized as per
	  ...
	  *
	  * [[net.noresttherein.slang.time.TimeSpan Fixed]] and [[net.noresttherein.slang.time.InfiniteTimeInterval infinite]]
	...
	/** Negates the length of this time extent. The [[net.noresttherein.slang.time.TimeExtent.fixed fixed]] part
	  * of this instance simply changes signs, while the [[net.noresttherein.slang.time.TimeExtent.variable variable]]
	  * part changes the sign on all calendar units. [[net.noresttherein.slang.time.InfiniteTimeInterval Infinite]]
	  * time extents return the opposite infinity.
	  */
	def unary_- :TimeExtent

	/** A time extent which is the sum of the two operand extents.
	  * [[net.noresttherein.slang.time.Period Period]]s are summed on the coordinates; due to their variable nature,
	...
	/** A time extent which is the signed difference between the two operand extents.
	  * [[net.noresttherein.slang.time.Period Period]]s are subtracted on the coordinates; due to their variable nature,
	  * `Time.now + this - other` may be unequal to `Time.now + (this - other)`.
	  */
	def -(other :TimeExtent) :TimeExtent
...
/** Base trait for measured finite, elapsed time in both fixed length time units and variable length calendar units.
  * Instances include [[net.noresttherein.slang.time.TimeSpan TimeSpan]] for time-based spans,
  * [[net.noresttherein.slang.time.DateSpan DateSpan]] for date-based spans,
...
/** Elapsed time of fixed length. This includes both infinite
  * [[net.noresttherein.slang.time.Eternity Eternity]]/[[net.noresttherein.slang.time.MinusEternity MinusEternity]]
  * and finite [[net.noresttherein.slang.time.TimeSpan TimeSpan]] subclasses. Instances of all subclasses,
  ...
  * is class-based, with [[net.noresttherein.slang.time.Milliseconds Milliseconds]]`(0)`,
  * [[net.noresttherein.slang.time.Duration Duration]]`.Zero` and [[net.noresttherein.slang.time.Immediate Immediate]]
  * all being unequal.
...
sealed trait TimeInterval extends Any with TimeExtent with Ordered[TimeInterval] {
	...
	/** The floor of the length of this time extent in seconds. The nanosecond remainder is available
	  * as the [[net.noresttherein.slang.time.TimeInterval.nanos nanos]] property.
	  * @throws UnsupportedOperationException if this extent is infinite.
	  */
	def toSeconds :Long
	...
	/** The remaining seconds in this time extent that do not fit into full minutes.
	  * The result is a floor from the division of [[net.noresttherein.slang.time.TimeInterval.toSeconds toSeconds]] by `60`.
	  * @throws UnsupportedOperationException if this extent is infinite.
	  */
	def seconds :Int

	/** The remaining minutes in this time extent that do not fit into full hours.
	  * The result is a floor from the division of [[net.noresttherein.slang.time.TimeInterval.toSeconds toSeconds]] by `3600`.
	  * @throws UnsupportedOperationException if this extent is infinite.
	  */
	def minutes :Int
	...
	/** The largest time unit (from
	  ...
	  * `Duration(this.`[[net.noresttherein.slang.time.TimeInterval.to to]]`(unit), unit) == this`.
	  * [[net.noresttherein.slang.time.InfiniteTimeInterval Infinite]] time extents return
...
/** Elapsed time measured in time-based units representing the ISO-8601 concept of ''duration''.
  * The default implementation is [[net.noresttherein.slang.time.Duration Duration]]; this trait exists
  * primarily to allow interoperability
  * with the lightweight span of [[net.noresttherein.slang.time.Milliseconds Milliseconds]]
  * and [[net.noresttherein.slang.time.Immediate Immediate]]
  * (a zero-length duration which is also a [[net.noresttherein.slang.time.DateSpan DateSpan]]).
  */
...
/** Proper, finite time extents measured in date-based units and thus having variable length, depending on
  ...
  * This trait is separate from `Period` primarily to allow [[net.noresttherein.slang.time.Immediate Immediate]]
  * to extend it.
  */
...
/** Base trait for the two infinite `TimeExtent` implementations: `Eternity` and `MinusEternity`.
  * Both are not only [[net.noresttherein.slang.time.TimeInterval TimeSpan]]s,
  * but also [[net.noresttherein.slang.time.DateInterval DateInterval]]s.
  */
