package net.noresttherein.slang.typist.casting
...
class SaferCasting[X](private val self :X) extends AnyVal {
	...
	/** Applies the given function to `this` if `this.isInstanceOf[T]`, returning the result in an [[net.noresttherein.slang.vars.Option]].
	  * {{{
