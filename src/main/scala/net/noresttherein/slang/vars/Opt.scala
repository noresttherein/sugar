...
import net.noresttherein.slang.vars.Opt.{unzip2Lack, unzip3Lack, Got, Lack, NoContent, WithFilter}
...
/** A value class treating nullable reference types as option-like monads erased in runtime. It behaves
  ...
  * [[net.noresttherein.slang.vars.Opt.Got$ Got]] matching pattern, which should by modern compilers be translated
	...
	extends AnyVal with Ref[A] with IterableOnce[A] with Product with Equals with Serializable
{
	...
	/** Tests if this `Opt` does not contain a value (is equal to [[net.noresttherein.slang.vars.Opt.Lack Lack]]). */
	...
	/** The wrapped value. It is the same as [[net.noresttherein.slang.vars.Opt.get get]], but this method
	  * is only possible to call on instances for which it is statically known that the value exists based on
	  * [[net.noresttherein.slang.vars.Opt.isEmpty! isEmpty]] member type.
	...
	/** Similarly to [[net.noresttherein.slang.vars.Opt.getOrElse getOrElse]], returns the value if non-empty
	...
	/** Gets the element in the `Opt` or throws the exception given as the type parameter with the given message.
	  ...
	  * @see [[net.noresttherein.slang.vars.Opt.orNoSuch orNoSuch]]
	  * @see [[net.noresttherein.slang.vars.Opt.orIllegal orIllegal]] */
	...
	/** Gets the element in this `Opt` or throws a [[NoSuchElementException]] with the given message.
	  * @see [[net.noresttherein.slang.vars.Opt.orThrow orThrow]] */
	...
	/** Similarly to [[net.noresttherein.slang.vars.Opt.orElse orElse]], returns this `Opt` if it is not empty
	  * and `alt` otherwise. The difference is that the alternative value is not lazily computed and guarantees
	...
	/** Returns the result of applying `f` to the value of this `Opt` if it is non empty,
	  ...
	  * [[net.noresttherein.slang.vars.Opt.mapOrElse mapOrElse]] instead.
	...
	/** Returns a new `Opt` containing this value if it is not empty and its value satisfies the given predicate,
	  * or [[net.noresttherein.slang.vars.Opt.Lack Lack]] otherwise. */
...
/** Companion object providing factory methods and extractors working with [[net.noresttherein.slang.vars.Opt Opt]]s.
  * @see [[net.noresttherein.slang.vars.Opt.Lack]]
  * @see [[net.noresttherein.slang.vars.Opt.Got$]]
  */
object Opt {
	/** Wraps the given reference in a purely syntactic option-like object erased in the runtime.
	  ...
	  * @see [[net.noresttherein.slang.vars.Opt]]
	  */
...
//		else x.asInstanceOf[Opt[T]]
	...
	/** A refinement of [[net.noresttherein.slang.vars.Opt Opt]] marking it through a member flag type
	  * as non-empty. [[net.noresttherein.slang.vars.Opt$ Opt]] factory object creates instances
	  * narrowed down to this type.
	  */
	type Got[+T] = Opt[T] { type isEmpty = false }

	/** Factory and a matching pattern for non empty values of [[net.noresttherein.slang.vars.Opt Opt]]. */
	object Got {
		/** Creates a non-empty [[net.noresttherein.slang.vars.Opt Opt]] wrapping the given value. */
		@inline def apply[T](x :T) :Got[T] = new Opt(x.asInstanceOf[AnyRef]).asInstanceOf[Got[T]]

		/** Matches non-empty [[net.noresttherein.slang.vars.Opt Opt]] instances. */
		@inline def unapply[T](opt :Opt[T]) :Opt[T] = opt
	}

	/** A refinement of [[net.noresttherein.slang.vars.Opt Opt]] marking it through a member flag type
	  * as empty. [[net.noresttherein.slang.vars.Opt.Lack Opt.Lack]] is an instance of this type.
	  */
	type Lack = Opt[Nothing] { type isEmpty = true }

	/** A special, empty instance of [[net.noresttherein.slang.vars.Opt Opt]] which conforms to any `Opt[T]` type.
	  ...
	  * applications.
	  * @see [[net.noresttherein.slang.vars.Opt.empty]]
	  */
