...
import net.noresttherein.slang.time.{Eternity, Immediate, Milliseconds, PosixTime, TimeInterval}
...
/** A thread safe [[net.noresttherein.slang.vars.InOut InOut]] variable performing all access to its value
  * while holding its monitor. Very similar to [[net.noresttherein.slang.vars.SyncVar SyncVar]],
  * but all updates are followed with a [[java.lang.Object.notifyAll notifyAll]]`()` call.
  * In addition to the standard API, this class defines a family of [[net.noresttherein.slang.vars.SignalVar.await await]]
  ...
  *
  * One significant difference from a similar [[net.noresttherein.slang.vars.SignalVal SignalVal]] -
  ...
  * @see [[net.noresttherein.slang.vars.SignalVal]]
  * @see [[net.noresttherein.slang.vars.Watched]]
  * @see [[net.noresttherein.slang.vars.Relay]]
  */
	...
	extends SyncVar[T]
{
	...
	/** The number of threads currently waiting in one of the `await` or `testAndAwait` methods to see the result of
	  ...
	  * See [[net.noresttherein.slang.vars.SignalVar.mutations mutations]] for a detailed description
...
//	}
	...
	/** Waits for the value of this variable to change.
	  * The change here means any call to setters [[net.noresttherein.slang.vars.SignalVar.value_= value]]` = _`,
	  * `this `[[net.noresttherein.slang.vars.InOut.:= :=]]` _` as well as
	  * [[net.noresttherein.slang.vars.SignalVar.testAndSet testAndSet]] or other atomic mutator methods.
	...
	/** Awaits until the value of this variable is changed.
	  ...
	  * this variable as in [[net.noresttherein.slang.vars.SignalVar.await await]]. Unlike in the latter method,
