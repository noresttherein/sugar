...
import net.noresttherein.slang.vars.Unsure.{collector, unzip2Fail, unzip3Fail, WithFilter}
...
/** A `@specialized` (for all value types with the exception of `Unit`) version of [[scala.Option]]
  ...
  * Like `Option`, it has two subclasses: [[net.noresttherein.slang.vars.Sure Sure]]`[T]` (equivalent of `Some[T]`)
  * and [[net.noresttherein.slang.vars.Blank Blank]] (equivalent of `None`).
  ...
  */
sealed trait Unsure[@specialized(SpecializedVars) +T] extends Ref[T] with IterableOnce[T] with Product with Serializable {
	/** Tests if this `Unsure` does not contain a value (is equal to [[net.noresttherein.slang.vars.Blank Blank]]). */
	@inline final def isEmpty: Boolean = this eq Blank

	/** Tests if this `Unsure` contains a value. If true, `get` will not throw an exception.
	  * Equivalent to [[net.noresttherein.slang.vars.Unsure.isDefined isDefined]].
	  */
	@inline final def nonEmpty: Boolean = this ne Blank

	@inline final override def isDefined :Boolean = this ne Blank

	@inline override def knownSize :Int = if (this eq Blank) 0 else 1

	@inline override def productArity :Int = if (this eq Blank) 0 else 1

	@inline override def productElement(n :Int) :Any =
		if (n == 1 && (this ne Blank)) get
		else throw new IndexOutOfBoundsException(toString + ".productElement(" + n + ")")

	/** Forces extraction of the value.
	  * @return contained value, if this is a [[net.noresttherein.slang.vars.Sure Sure]].
	...
	/** Similarly to [[net.noresttherein.slang.vars.Unsure.getOrElse getOrElse]], returns the value if non-empty
	...
	@inline final def orNull[O >: T](implicit isNullable :Null <:< O) :O =
		if (this eq Blank) null.asInstanceOf[O] else get
	...
	/** Gets the value of this instance or throws the exception given as the type parameter with the given message.
	  ...
	  * @see [[net.noresttherein.slang.vars.Unsure.orNoSuch orNoSuch]]
	  * @see [[net.noresttherein.slang.vars.Unsure.orIllegal orIllegal]]
	  */
	@inline final def orThrow[E <: Throwable :ClassTag](msg: => String) :T =
		if (this eq Blank) raise[E](msg) else get

	/** Gets the value of this instance or throws a [[NoSuchElementException]] with the given message.
	  * @see [[net.noresttherein.slang.vars.Unsure.orThrow orThrow]]
	  */
	@inline final def orNoSuch(msg: => String) :T =
		if (this eq Blank) throw new NoSuchElementException(msg) else get
	...
	@inline final def orIllegal(msg: => String) :T =
		if (this eq Blank) throw new IllegalArgumentException(msg) else get
	...
	@inline final def orElse[O >: T](alt: => Unsure[O]) :Unsure[O] =
		if (this eq Blank) alt else this

	/** Similarly to [[net.noresttherein.slang.vars.Unsure.orElse orElse]], returns this `Unsure` if it is not empty
	  * and `alt` otherwise. The difference is that the alternative value is not lazily computed and guarantees
	...
	/** Returns `this` ''iff'' this value is [[net.noresttherein.slang.vars.Blank Blank]],
	  * or a [[net.noresttherein.slang.vars.Sure Sure]] containing the result of applying
	  * the given function to its value otherwise.
	  */
	@inline final def map[O](p :T => O) :Unsure[O] =
		if (this eq Blank) Blank else new Sure(p(get))
	...
	@inline final def mapOrElse[X](f :T => X, alternative: => X) :X =
		if (this eq Blank) alternative else f(get)
	...
	/** Returns the result of applying `f` to the value of this `Unsure` if it is non empty,
	  ...
	  * [[net.noresttherein.slang.vars.Unsure.mapOrElse mapOrElse]] instead.
	  *  @param  ifEmpty the expression to evaluate if empty.
	  *  @param  f       the function to apply if nonempty.
	  */
	@inline final def fold[B](ifEmpty: => B)(f: T => B): B =
		if (this eq Blank) ifEmpty else f(get)
	...
	/** Equivalent to `this.`[[net.noresttherein.slang.vars.Unsure.filter filter]]`(p)` - a variant for use
	...
	@inline final def exists(p :T => Boolean): Boolean = (this ne Blank) && p(get)

	/** Tests if this `Unsure` contains no value or its value satisfies the given predicate. */
	@inline final def forall(p :T => Boolean): Boolean = (this eq Blank) || p(get)

	/** Executes the given block for this `Unsure`s value if it is not empty. */
	@inline final def foreach[O](f :T => O) :Unit = if (this ne Blank) f(get)
	...
	def collect[O](f :PartialFunction[T, O]) :Unsure[O] =
		if (this eq Blank)
			...
			if (res.asInstanceOf[AnyRef] eq collector) Blank else new Sure(res)
		}
	...
	@inline final def zip[O](that :Unsure[O]) :Unsure[(T, O)] =
		if ((this eq Blank) | (that eq Blank)) Blank
	...
	@inline final override def iterator :Iterator[T] =
		if (this eq Blank) Iterator.empty else Iterator.single(get)

	/** Returns `Nil` if this `Unsure` is empty or or `this.get::Nil` otherwise. */
	@inline final def toList :List[T] = if (this eq Blank) Nil else get::Nil

	/** Returns an empty list if this `Unsure` is empty or a single element list with its value otherwise. */
	@inline final def toSeq :Seq[T] = if (this eq Blank) Nil else get::Nil

	/** Returns an empty collection if this `Unsure` is empty or a singleton with its value otherwise. */
	@inline final def toIterable :Iterable[T] = if (this eq Blank) Iterable.empty else Iterable.single(get)

	/** Conversion to standard Scala [[scala.Option]].
	  * @return `Some(this.get)` if `this.nonEmpty` or `None` otherwise.
	  */
	override def ? :Option[T] = None //overriden by Success

	override def unsure :Unsure[T] = this
	...
	@inline final def toLeft[O](right: => O) :Either[T, O] =
		if (this eq Blank) Right(right) else Left(get)
	...
	@inline final def toRight[O](left: => O) :Either[O, T] =
		if (this eq Blank) Left(left) else Right(get)
...
/** A factory of `Option`-like values [[net.noresttherein.slang.vars.Unsure Unsure]]`[T]`, providing additionally
  * implicit conversions to and from [[scala.Option]], [[net.noresttherein.slang.vars.Opt]] and [[Iterable]].
  */
object Unsure {

	/** An `Unsure` factory which creates [[net.noresttherein.slang.vars.Sure Sure]]`(value)`
	  * if the argument is not null, and [[net.noresttherein.slang.vars.Blank Blank]] if it is null.
	...
	def some_?[@specialized(SpecializedVars) T](value :Option[T]) :Unsure[T] =
		if (value.isDefined) new Sure(value.get, value) else Blank

	/** Converts the given `Opt[T]` into a ''specialized'' `Unsure[T]` instance. */
	def got_?[@specialized(SpecializedVars) T](value :Opt[T]) :Unsure[T] =
		if (value.isDefined) new Sure(value.get, cachedOpt = value) else Blank

	/** Returns [[net.noresttherein.slang.vars.Blank Blank]] in a manner consistent with the collections hierarchy. */
	def empty[T] : Unsure[T] = Blank

	/** When a given condition is true, evaluates the `value` argument and returns
	  * [[net.noresttherein.slang.vars.Sure Sure]]`(value)`. When the condition is false, `value` is not evaluated
	  * and [[net.noresttherein.slang.vars.Blank Blank]] is returned.
	  */
	def when[@specialized(SpecializedVars) T](cond: Boolean)(value: => T): Unsure[T] =
		if (cond) new Sure(value) else Blank

	/** Unless a given condition is true, this will evaluate the `value` argument and
	  * return [[net.noresttherein.slang.vars.Sure Sure]]`(value)`. Otherwise, `value` is not evaluated
	  * and [[net.noresttherein.slang.vars.Blank Blank]] is returned.
	  */
	...
	/** The for-comprehension facade for [[net.noresttherein.slang.vars.Unsure Unsure]]`[A]`
...
//	/** Provides [[net.noresttherein.slang.vars.Unsure.OptionToUnsureConverter.toUnsure toUnsure]] extension method
//	  * for any `Option[T]`, converting it to a `Unsure[T]`. A fully implicit conversion is available for import
//	  * as `Unsure.implicits.`[[net.noresttherein.slang.vars.Unsure.implicits.optionToUnsure optionToUnsure]].
...
//
	...
	object implicits {
		...
		/** An implicit conversion from an `Unsure[A]` to an `Option[A]`.
		  ...
		  */
		@inline implicit def unsureToOption[T](value :Unsure[T]) :Option[T] = value.asOption
	...
	/** Importing the contents of this object replace all usage of [[Option]]/[[Some]]/[[None]] in the scope with
	  * [[net.noresttherein.slang.vars.Unsure Unsure]]/[[net.noresttherein.slang.vars.Sure Sure]]/[[net.noresttherein.slang.vars.Blank Blank]].
	...
	object UnsureAsOption {
		...
		@inline implicit def unsureToOption[T](opt :Unsure[T]) :scala.Option[T] = opt.?
		...
		@inline implicit def sureToSome[@specialized(SpecializedVars) T](opt :Sure[T]) :scala.Some[T] =
			opt.?.asInstanceOf[scala.Some[T]]

		@inline implicit def noneToBlank(none :scala.None.type) :Blank.type = Blank
		@inline implicit def blankToNone(blank :Blank.type) :scala.None.type = scala.None
	}
	...
	private val unzip2Fail = (Blank, Blank)
	private val unzip3Fail = (Blank, Blank, Blank)
}
...
/** An `Unsure[T]` carrying a value, a counterpart of Scala [[Some]].
  ...
  */
@SerialVersionUID(1L)
final class Sure[@specialized(SpecializedVars) +T] private[vars]
                (x :T, private[this] var cachedOption :Option[T] = None, private[this] var cachedOpt :Opt[T] = Lack)
	extends Unsure[T]
{
	/** Same as [[net.noresttherein.slang.vars.Sure.get get]], for compatibility with [[Some]].
	  * Usage of `value` instead of `get` or `apply()` guarantees that the receiver is a `Sure` instance
	  * rather than a, possibly empty, [[net.noresttherein.slang.vars.Unsure Unsure]].
...
/** A factory and deconstructor of full (''sure'') [[net.noresttherein.slang.vars.Unsure Unsure]] instances,
...
/** An empty (''blank'') [[net.noresttherein.slang.vars.Unsure Unsure]] instance, a counterpart of [[scala.None]]. */
@SerialVersionUID(1L)
case object Blank extends Unsure[Nothing] {
	override def get :Nothing = throw new NoSuchElementException("Blank()")
