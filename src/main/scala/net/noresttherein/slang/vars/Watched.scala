...
import net.noresttherein.slang.vars.Watched.SerializedExecutor
import net.noresttherein.slang.witness.{DefaultValue, Maybe}




/** A [[net.noresttherein.slang.vars.Volatile Volatile]]-like variable which additionally executes callback actions
  ...
  * and synchronous watchers, registered with methods [[net.noresttherein.slang.vars.Watched.watch watch]]
  * and [[net.noresttherein.slang.vars.Watched.inSync inSync]], respectively. When and how the former are executed
  * in respect to the assignment call and each other is unspecified and depends solely on the provided `Executor`
  * implementation. While the default [[net.noresttherein.slang.vars.Watched.SerializedExecutor SerializedExecutor]]
  ...
  *
  * This variable kind is somewhat similar in application to [[net.noresttherein.slang.vars.SignalVar SignalVar]]
  ...
  *
  * @see [[net.noresttherein.slang.vars.Watched.value_=]]
...
//	@scala.volatile private[this] var synchronousWatchers :Set[T => Unit] = Set.empty // Watcher.watchersField
	...
	/** Assigns a new value to this variable and ensures all registered watchers are executed.
	  ...
	  * (rather than `this.`[[net.noresttherein.slang.vars.Watched.value value]] at the time the watcher is called.
...
//			synchronousWatchers = new ConcurrentSkipListSet[T => Unit].asScala
	...
	/** Removes the given callback from the list of watchers of this variable.
	  ...
	  * to [[net.noresttherein.slang.vars.Watched.watch watch]].
	  */
...
object Watched extends VolatileLikeCompanion[Watched] {
	...
	/** Create a new $variable which can be shared and watched by multiple threads. The implicit
	  ...
	  * If no implicit value exists, [[net.noresttherein.slang.vars.Watched.SerializedExecutor SynchronousExecutor]]
	...
	/** Create a new $variable which can be shared and watched by multiple threads, initialized with the default
	  ...
	  * [[net.noresttherein.slang.vars.Watched.SerializedExecutor SerializedExecutor]] will be used instead.
	  */
