...
import net.noresttherein.slang.witness
import net.noresttherein.slang.witness.Maybe.NoImplicit
...
object Maybe extends MaybeNoImplicit {
	/** A wrapper type for a type constructor `F` allowing to use a `Maybe` for a generic type as a type class
	  ...
	  */
	type TypeClass[F[_]] = {
		/** A type constructor for [[net.noresttherein.slang.witness.Maybe Maybe]]`[F[T]]`,
		  * where `F` is a type class/type constructor specified as the preceding argument to `TypeCass`.
		  */
		type Maybe[X] = witness.Maybe[F[X]]
	}

	/** A shorter alternative for [[net.noresttherein.slang.witness.Maybe.TypeClass TypeClass]]`[F]` composing
	  * [[net.noresttherein.slang.witness.Maybe! Maybe]] with a specified a type constructor
