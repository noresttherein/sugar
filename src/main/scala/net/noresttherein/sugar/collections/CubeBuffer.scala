...
import net.noresttherein.sugar.collections.CubeBuffer.{Cube, Cube1, Cube1Dimension, Cube1Ops, Cube2, Cube2Dimension, Cube2Ops, Cube3, Cube3Dimension, Cube3Ops, Cube4, Cube4Dimension, Cube4Ops, Cube5, Cube5Dimension, Cube5Ops, CubeBufferIterator, CubeDimension, CubeOps, DimBits, DimMask, DimMask1, InitialCapacity, InitialCapacityLog, Items, IterableOnceItems, MaxEdge, MaxSize1, MaxSize2, MaxSize3, MinCapacity, SingleItem, project, remainder, side1, side2}
import net.noresttherein.sugar.collections.util.hasFastDrop
import net.noresttherein.sugar.extensions.{AnyRefExtension, ArrayExtension, ArrayObjectExtension, BooleanExtension, ClassExtension, IntExtension, IterableOnceExtension, LongExtension, castTypeParamMethods, castingMethods, classNameMethods}
	   ...
	   with SugaredIterable[E] with SugaredIterableOps[E, CubeBuffer, CubeBuffer[E]]
{
	...
	/** An N-dimensional array (`N=`[[net.noresttherein.sugar.collections.CubeBuffer.dim dim]]) of `E`. Its length
	  * is always between [[net.noresttherein.sugar.collections.sugar.CubeBuffer.InitialCapacity InitialCapacity]]
	  ...
	  * for a buffer of the first dimension, it is also a multiplicity of `InitialSize`, while the capacity of
	  * higher dimensions is always divisible by `MaxEdge`.
	  *
...
//		val cube               = this.cube
		...
		prefixDim += 1
		prefixCubeSize <<= DimBits
		val PrefixCubeMask   = prefixCubeSize - 1
		...
		var d = dim
		var mask = lowerCubeSize - 1L //(1L << dim) - 1L
		var pref :Array[_] = null
		var suff :Array[_] = null
		var border :Array[_] = data
		var cubeSize :Long = lowerCubeSize
//		var buff = new CubeBuffer[E]
		while ((absoluteIdx & mask) != 0L) {
			d -= 1
			val prefixIsSingleCube = cubeSize > idx + (dataOffset & mask)
			val suffixIsSingleCube = cubeSize > watermark - (absoluteIdx & mask)
			if (prefixIsSingleCube)

			if (cubeSize > idx + (dataOffset & mask)) {
				pref = border((dataOffset >> d * DimBits).toInt).asInstanceOf[Array[_]]
			}
			if (cubeSize > watermark - (absoluteIdx & mask)) {
				suff = border((absoluteIdx >> d * DimBits).toInt).asInstanceOf[Array[_]]
			}
		}


		if (prefixDim < suffixDim || prefixDim == suffixDim && prefixDim < dim)
			//We can fit the whole prefix, without shifting, in a single cube of a lower dimension than this one,
			// and not greater than the cube size required to fit the suffix. Lets extract the prefix as a stand alone
			...
			if ((absoluteIdx & PrefixCubeMask) == absoluteIdx) {
				//We are lucky! idx falls exactly on the boundary of the smallest cube containing prefix.
				val emptyFrontSpace = prefixCubeOffset - storageOffset //empty, allocated slots preceding the cube with the prefix
				//Shift empty, allocated space preceding the prefix cube by prefixCubeSize right, but do it smart:
				// don't shift empty space, just reallocate the minimum required for contingency.
				val suffix       = cube
				val suffixDim    = dim
				val suffixEnd    = storageEnd
				val suffixOffset =
					if (storageOffset >= prefixCubeOffset)
						0L
					else {
						(prefixCubeOffset - storageOffset) >> (DimBits * prefixDim) match {
							case 0L => 0L
							case 1L =>
								thisDimension.move(
									suffix, storageOffset, suffix, storageOffset + prefixCubeSize, emptyFrontSpace
								)
								storageOffset + prefixCubeSize
							case  n =>
								val fullEmptyCubeOffset = storageOffset + prefixCubeSize - 1 & PrefixCubeMask
								val emptyFirstCubeSpace = emptyFrontSpace & PrefixCubeMask
								thisDimension.move(
									suffix, fullEmptyCubeOffset, suffix, prefixCubeOffset, prefixCubeSize
								)
								thisDimension.move(
									suffix, storageOffset, suffix, storageOffset + prefixCubeSize, emptyFirstCubeSpace
								)
								storageOffset + prefixCubeSize
						}
					}
				this.data = prefix
				storageOffset = if (emptyFrontSpace >= 0L) 0L else storageOffset & PrefixCubeMask
				...
				if (dim < suffixDim) { //now prepend this.cube to suffix. The dimension is still lower, so it will fit.
					val roundedDataEnd = dataOffset + _size + MaxSize1 - 1 & ~DimMask1
					val emptyBackSpace = storageEnd - roundedDataEnd
					if (emptyBackSpace > 0) { //move any empty space out of the way
						if (emptyBackSpace <= storageOffset) {
							cubeDimension.move(cube, roundedDataEnd, cube, storageOffset - emptyBackSpace, emptyBackSpace)
							storageEnd -= emptyBackSpace
							storageOffset -= emptyBackSpace
						} else if (emptyBackSpace <= maxCapacity - suffixEnd) {
							var data :MyCube[E] = suffix
							var d = suffixDim
							while (d > dim) {
								data = data(project(suffixEnd, d)).asInstanceOf[MyCube[E]]
								d -= 1
							}
							cubeDimension.move(cube, roundedDataEnd, data, remainder(suffixEnd, d), emptyBackSpace)
						} //todo: other cases
					}
					var data :Array[Any] = suffix.asInstanceOf[Array[Any]]
					var d = suffixDim
					while (d > dim + 1) {
						data = data(0).asInstanceOf[Array[Any]]
						d -= 1
					}
					data(0) = this.data
					val prefixCapacity = storageEnd - storageOffset
					dataOffset = dataOffset - storageOffset + suffixOffset - prefixCapacity
					thisDimension.move(suffix, 0, suffix, suffixOffset - prefixCapacity, prefixCapacity)
					if (_size < suffixSize)
						thisDimension.shiftRightInPlace(suffix, dataOffset,  dataOffset + _size, suffixOffset)
					else
						thisDimension.shiftLeftInPlace(suffix, dataOffset + _size, suffixOffset, suffixOffset + suffixSize)
				}
		...
		if ((absoluteIdx & lowerCubeSize - 1) == 0L) { //We are lucky! idx is the first index in a cube^(dim-1)
			if (idx <= lowerCubeSize) { //Even more lucky, prefix fits in a single cube^(dim-1)!
				val data = this.data.asInstanceOf[Array[Any]]
				val prefixCubeIdx = (absoluteIdx >> lowerDimensionBits).toInt
				val prefixCube = data(prefixCubeIdx)
				val suffix = cube
				val firstStorageCube = (storageOffset >> lowerDimensionBits).toInt
				//shift any allocated, empty cubes in data one position up.
				(prefixCubeIdx - firstStorageCube) match {
					case 0 =>
					case 1 => data(prefixCubeIdx) = suffix(firstStorageCube)
					case n =>
						data(prefixCubeIdx) = suffix(firstStorageCube + 1)
						data(firstStorageCube + 1) = suffix(firstStorageCube)
						data(firstStorageCube) = null
				}
				this.data = prefixCube.asInstanceOf[Array[_]]
			}
		}
		//The number of last elements in prefix we'll need to temporarily copy if we retain the data array
		// containing idx with the suffix, and create a new one, appended to prefix
		val prefixPadding =
			if ((dataOffset >> DimBits) == (absoluteIdx - 1 >> DimBits)) idx
			else absoluteIdx & DimBits
		val suffixPadding =
			if ((absoluteIdx >> DimBits) == (watermark - 1 >> DimBits)) suffixSize


//				prefixCubeSize <<= DimBits
		//The total number of cubes in prefix, of all dimensions, following the last boundary of cube (dim - 1).
		// This is the total number of array elements which will be copied if
		var trailingPrefixCubes = 0
		while (prefixCubeSize > 1L) {
			trailingPrefixCubes += ((absoluteIdx & prefixCubeSize - 1) - (dataOffset & prefixCubeSize - 1)).toInt
			prefixCubeSize >>= DimBits
		}

		val trailingDim1CubesInPrefix = absoluteIdx + MaxSize1 - 1 >> DimBits

		var suffixCube1 :Cube[_] = null
		var suffixCube2 :Cube[_] = null
		var suffixCube3 :Cube[_] = null
		???
	}
...
object CubeBuffer extends StrictOptimizedClassTagSeqFactory[CubeBuffer] {
	...
	@inline final def newCapacity(min :Int) :Int =
		if (min > MaxEdge)
...
//		def slice[E](cube :C[E], from :Long, until :Long) :C[E]
		...
		/** Allocate (in place) lower dimensional arrays (`C[E]` and smaller) within `cube`, to grow the capacity
		  * from `currentCapacity` to `newCapacity`, counting in individual elements `E`. The arrays are allocated at
		  * [[net.noresttherein.sugar.collections.CubeBuffer.CubeDimension.project projections]] of indices within
		  * `[currentCapacity, newCapacity)` range onto this dimension. Existing contents are not affected.
		  *
		  ...
		  * @param cube  top level array, in which new lower level arrays are allocated.
		...
		/** Splits the argument cube into two parts, and returns a cube with values from the range `[from, until)`,
		  * after erasing it from the argument cube. If the index range falls exactly at the start of a cube
		  ...
		  * copied recursively. If, in certain sub-cube `C` of the given cube, `[from, until)` range is covered by
		  ...
		  * of `InitialCapacity` (and a power of `2`) greater than `n`. The first `n+1` positions in the returned array
		  * will be occupied by cubes `c0', c1, ..., cn'`, where `c0'` and `cn'` are prefix and suffix cubes of
		  * `c0` and `cn`, respectively, and retain both the maximum capacity (all arrays on all dimensions
		  * are of length `MaxEdge`) and alignment of cubes `c0` and `cn`. The positions occupied by cubes `c0` and `cn`
		  * in cube `C` are set to cubes `c0''` and `cn''`, complementary to the returned cubes, while positions.
		  * used by cubes `c2,...c{n-1}` are set to `null`.
		  */
		def sliceInPlace[E](cube :C[E], from :Long, until :Long) :Array[_] =
			sliceInPlace(cube, from, until, -1, false, false)

		def sliceInPlace[E](cube :C[E], from :Long, until :Long, align :Int, retainCapacity :Boolean, retainDimension :Boolean) :Array[_]
		...
		/** Copies data from absolute index range `[srcPos, srcPos + length)` in `src` cube, to range
		  ...
		  * [[net.noresttherein.sugar.collections.CubeBuffer.CubeDimension.shiftInPlace shiftInPlace]], and whole
		  * the positions of whole cubes are swapped in the arrays, without copying the contents of any arrays
		  * of the lowest dimension. The source index range must be covered by allocated space, but it is not required
		...
		/** Moves the elements from the range `[srcPos, srcPos + length]` of cube `src`
		  ...
		  * also ranges in arrays of dimensions between `1..Dim` (exclusive).
		  */
	...
	private implicit object Cube1Dimension extends CubeDimension[Cube1](1) {
		...
		//		override def slice[E](cube :Cube1[E], from :Long, until :Long) :Cube1[E] = cube.slice(from.toInt, until.toInt)
		...
		override def sliceInPlace[E](cube :Cube1[E], from :Long, until :Long, align :Int,
		                             retainCapacity :Boolean, retainDimension :Boolean)
				:Array[_] =
			if (from <= 0 && until >= cube.length)
				...
				val capacity = if (retainCapacity) cube.length else newCapacity(len)
				val res = Array.make(cube.getClass.getComponentType.castParam[E], capacity)
				val pos = align match {
					case -1 => 0
					case  0 => from
					case  1 => capacity - len
...
//		override def slice[E](cube :Array[C[E]], from :Long, until :Long) :Array[C[E]] =
//			if (until <= from)
//				make[E](InitialCapacity)
//			else {
//				val fromN  = project(from)
//				val fromL  = relativeIdx(from)
//				val untilN = project(until - 1L)
//				val untilL = relativeIdx(until - 1L) + 1L
//				if (fromN == untilN) {
//					val res = make[E](InitialCapacity)
//					res(0) = lower.slice(cube(fromN), fromL, untilL)
//					res
//				} else {
//
//				}
//			}
		...
		override def sliceInPlace[E](cube :Array[C[E]], from :Long, until :Long, align :Int,
		                             retainCapacity :Boolean, retainDimension :Boolean)
				:Array[_] =
		{
			val fromN  = project(from)
			val fromL  = relativeIdx(from)
			val untilN = project(until - 1)
			val untilL = relativeIdx(until - 1) + 1
			if (fromN == untilN && !retainDimension && !retainCapacity)
				lower.sliceInPlace(cube(fromN), fromL, untilL, align, false, false)
			else if (fromL != 0L | untilL != 0L)
				throw new IllegalArgumentException(
					"Cannot slice a non aligned range [" + from + ", " + until + ") (indices [" +
						fromL + ".." + untilL + "]) of " + Dim + "-dimensional cube " + cube.getClass)
			else {
				val cubeCount      = untilN + 1 - fromN
				val fullCubesFrom  = project(from + MaxSize1 - 1L)
				val fullCubesUntil = project(until)
				val capacity       = if (retainCapacity) cube.length else newCapacity(cubeCount)
				val pos = align match {
					case -1 => 0
					case  0 => fromN
					case  1 => capacity - cubeCount
				}
				val res = Array.copyOfRange(cube, fromN, untilN, pos, capacity)
				if (fullCubesFrom < fullCubesUntil)
					cube.clear(fullCubesFrom, fullCubesUntil)
				???
			}
		}
...
//				val untilL  = relativeIdx(until)
		...
		override def move[E](src :Array[C[E]], srcPos :Long, dst :Array[C[E]], dstPos :Long, length :Long) :Unit =
			if (length > 0) {
				...
				val dstN    = project(srcPos)
				val dstL    = relativeIdx(dstPos)
				val lengthN = project(length - 1) + 1
				if (srcL == 0 & dstL == 0)
