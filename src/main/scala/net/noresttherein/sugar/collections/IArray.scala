...
import net.noresttherein.sugar.collections.extensions.{ArrayExtension, ArrayLikeExtension, ArrayObjectExtension, IndexedSeqExtension, IterableExtension, IterableOnceExtension, SeqExtension}
import net.noresttherein.sugar.extensions.{ClassExtension, IteratorExtension, cast2TypeParamsMethods, cast3TypeParamsMethods, castTypeParamMethods, castingMethods, classNameMethods, downcast2TypeParamsMethods}
...
//	@inline final def copyAs[E :ClassTag](src :Array[_], from :Int)
	...
	/** Extension methods for all [[net.noresttherein.sugar.collections.ArrayLike ArrayLike]]`[E]` implementations.
	  ...
	  * @see [[net.noresttherein.sugar.collections.ArrayLike.RefArrayLikeExtension]] - additional extension methods
	...
	{   //avoid casting array to Array[E], as it may easily cause problems if inlined
		...
		@inline def indexOfSlice[A >: E](that :collection.Seq[A]) :Int =
			WrappedArray(array).castParam[E].indexOfSlice(that)

		@inline def indexOfSlice[A >: E](that :collection.Seq[A], from :Int) :Int =
			WrappedArray(array).castParam[E].indexOfSlice(that, from)

		@inline def lastIndexOfSlice[A >: E](that :collection.Seq[A]) :Int =
			WrappedArray(array).castParam[E].lastIndexOfSlice(that)

		@inline def lastIndexOfSlice[A >: E](that :collection.Seq[A], end :Int) :Int =
			WrappedArray(array).castParam[E].lastIndexOfSlice(that, end)

		@inline def indexOfSlice[A >: E](that :ArrayLike[A]) :Int =
			WrappedArray(array).castParam[E].indexOfSlice(Wrapped(that), 0)

		@inline def indexOfSlice[A >: E](that :ArrayLike[A], from :Int) :Int =
			WrappedArray(array).castParam[E].indexOfSlice(Wrapped(that), from)

		@inline def lastIndexOfSlice[A >: E](that :ArrayLike[A]) :Int =
			WrappedArray(array).castParam[E].lastIndexOfSlice(Wrapped(that), 0)

		@inline def lastIndexOfSlice[A >: E](that :ArrayLike[A], end :Int) :Int =
			WrappedArray(array).castParam[E].lastIndexOfSlice(Wrapped(that), end)

		@inline def containsSlice[A >: E](that :collection.Seq[A]) :Boolean =
			WrappedArray(array).castParam[E].indexOfSlice(that, 0) >= 0

		@inline def containsSlice[A >: E](that :ArrayLike[A]) :Boolean =
			WrappedArray(array).castParam[E].indexOfSlice(Wrapped(that), 0) >= 0


		@inline def corresponds[A](that :IterableOnce[A])(p :(E, A) => Boolean) :Boolean =
			WrappedArray(array).castParam[E].corresponds(that)(p)

		@inline def corresponds[A](that :ArrayLike[A])(p :(E, A) => Boolean) :Boolean =
			WrappedArray(array).castParam[E].corresponds(Wrapped(that))(p)

		@inline def sum[A >: E](implicit num :Numeric[A]) :A = WrappedArray(array).castParam[E].sum[A]
		@inline def product[A >: E](implicit num :Numeric[A]) :A = WrappedArray(array).castParam[E].product[A]
		@inline def min[A >: E](implicit ord :Ordering[A]) :E = WrappedArray(array).castParam[E].min[A]
		@inline def max[A >: E](implicit ord :Ordering[A]) :E = WrappedArray(array).castParam[E].max[A]
		@inline def minOption[A >: E](implicit ord :Ordering[A]) :Option[E] = WrappedArray(array).castParam[E].minOption[A]
		@inline def maxOption[A >: E](implicit ord :Ordering[A]) :Option[E] = WrappedArray(array).castParam[E].maxOption[A]
		@inline def minBy[A](f :E => A)(implicit cmp :Ordering[A]) :E = WrappedArray(array).castParam[E].minBy(f)
		@inline def maxBy[A](f :E => A)(implicit cmp :Ordering[A]) :E = WrappedArray(array).castParam[E].maxBy(f)
		@inline def minByOption[A](f :E => A)(implicit cmp :Ordering[A]) :Option[E] =
			WrappedArray(array).castParam[E].minByOption(f)

		@inline def maxByOption[A](f :E => A)(implicit cmp :Ordering[A]) :Option[E] =
			WrappedArray(array).castParam[E].maxByOption(f)
...
//	}
	...
	/** A factory and pattern for non-immutable collections wrapping arrays. */
	object Wrapped {
		def apply[A](array :ArrayLike[A]) :collection.IndexedSeq[A] = {
			val arr = array.castFrom[ArrayLike[A], Array[A]]
			ArraySliceSeq(arr, 0, arr.length)
		}
		...
		def unapply[A](items :IterableOnce[A]) :Opt[ArrayLike[A]] = items match {
			...
			case seq :AbstractArraySlice[A] if !seq.isImmutable => Got(seq.unsafeArray.castFrom[Array[_], ArrayLike[A]])
			case _ => Lack
		}
		...
		object Slice {
			def apply[A](array :ArrayLike[A], from :Int, until :Int) :collection.IndexedSeq[A] =
				ArraySliceSeq[A](array.castFrom[ArrayLike[A], Array[A]], from, until)
			...
			def unapply[A](items :IterableOnce[A]) :Opt[(ArrayLike[A], Int, Int)] = items match {
				...
				case seq :AbstractArraySlice[A] if !seq.isImmutable =>
...
case object IArray extends /*IArrayRank1Implicits with*/ ClassTagIterableFactory[IArray] {
		...
		extends AnyVal
	{
		...
		/** An 'exploded' variant of `updatedAll`: a copy of this array with elements starting at `index` substituted
		  ...
		  */
		@inline def updated[U >: E :ClassTag](index :Int, first :U, second :U, rest :U*) :IArray[U] =
			array.updated(index, first, second, rest :_*).castFrom[Array[U], IArray[U]]
		...
		@inline def inserted[A >: E :ClassTag](index :Int, first :A, second :A, rest :A*) :IArray[A] =
			array.inserted(index, first, second, rest :_*).castFrom[Array[A], IArray[A]]
		...
		@inline def appended[A >: E :ClassTag](first :A, second :A, rest :A*) :IArray[A] =
			array.appended(first, second, rest :_*).castFrom[Array[A], IArray[A]]
		...
		@inline def prepended[A >: E :ClassTag](first :A, second :A, rest :A*) :IArray[A] =
			array.prepended(first, second, rest :_*).castFrom[Array[A], IArray[A]]
	...
	override def from[E :ClassTag](it :IterableOnce[E]) :IArray[E] = it match {
		...
		case elems :Iterable[E] if elems.isEmpty => Array.empty[E].castFrom[Array[E], IArray[E]]
		case iter  :Iterator[E] if !iter.hasNext => Array.empty[E].castFrom[Array[E], IArray[E]]
	...
	override def empty[E :ClassTag] :IArray[E] = ArrayAsSeq.empty[E].castFrom[Array[E], IArray[E]]

	/** An empty array of the specified element type. */
	def empty[E](elementType :Class[E]) :IArray[E] = ArrayAsSeq.empty(elementType).castFrom[Array[E], IArray[E]]
	...
	def copyAs[E :ClassTag](array :ArrayLike[_], newLength :Int) :IArray[E] =
		ArrayAsSeq.copyAs[E](array.castParam[E], newLength).castFrom[Array[E], IArray[E]]
...
//		ArrayAsSeq.copyAs[E](array, array.length).castFrom[Array[E], IArray[E]]
	...
	/** Copies one array to another, truncating or padding with default values (if necessary) so the copy has
	  ...
	  */
	def copyOf[E :ClassTag](array :ArrayLike[E], newLength :Int) :IArray[E] =
...
//		ArrayAsSeq.copyAs[E](array, offset, newLength).castFrom[Array[E], IArray[E]]
	...
	def copyOf[E](array :IArray[E], newLength :Int) :IArray[E] =
		if (newLength == 0)
			...
			Array.copyOf(array.castFrom[IArray[E], Array[E]], newLength).castFrom[Array[E], IArray[E]]
...
//		}
	...
	/** Creates an `Array[E]` of the specified element type and length, executes the given initialization function
	...
	@inline final def make[E](elemType :Class[E], length :Int)(f :Array[E] => Unit) :IArray[E] = {
		val res = Array.make(elemType, length)
		f(res)
	    res.castFrom[Array[E], IArray[E]]
	}

	/** Creates a new `Array[E]` of the specified length and of the same type as the argument,
	...
	private[this] val isSeqPrototype = new IsSeq[IArray[Any]] with Serializable {
		...
		override def apply(array :IArray[Any]) = new IndexedSeqOps[Any, Seq, IArray[Any]] {
			...
			override def apply(i :Int) = array.castFrom[IArray[Any], Array[Any]].apply(i)
			...
			override def toIterable = IArray.Wrapped(array)
	...
	/** Wraps and unwraps immutable `IndexedSeq` instances and other immutable collections backed by arrays
	  ...
	  */
	object Wrapped {
		def apply[A](array :IArray[A]) :IndexedSeq[A] =
			PassedArrayWrapper(array) match {
				case Got(seq) => seq
				case _ => ArraySeq.unsafeWrapArray(array.castFrom[IArray[A], Array[A]])
			}
		...
		/** Factory of views on slices of immutable arrays as indexed sequences,
		  ...
		  */
		object Slice {
			def apply[A](array :IArray[A], from :Int, until :Int) :IndexedSeq[A] =
				PassedArrayWrapper.Slice(array, from, until) getOrElse ArraySliceSeq.Immutable(array, from, until)
...
object RefArrayLike {
	...
	class RefArrayLikeExtension[E, Arr[_]] private[collections] (private val array :Array[Any]) extends AnyVal {
		...
		/** An 'exploded' variant of `updatedAll`: a copy of this array with elements starting at `index` substituted
		  * by `first`, `second`, and contents of `rest`. The index must lie in range `[0..length - rest.size - 2)`.
		  */
		@inline def updated[A >: E](index :Int, first :A, second :A, rest :A*) :Arr[A] =
			array.updated(index, first, second, rest :_*).castFrom[Array[Any], Arr[A]]
		...
		/** An 'exploded' variant of `updatedAll`: a copy of this array with elements starting at `index` substituted
		  * by `first`, `second`, and contents of `rest`. The index must lie in range `[0..length)`.
		  */
		@inline def inserted[A >: E](index :Int, first :A, second :A, rest :A*) :Arr[A] =
			array.inserted(index, first, second, rest :_*).castFrom[Array[Any], Arr[A]]
		...
		@inline def appended[A >: E](first :A, second :A, rest :A*) :Arr[A] =
			array.appended(first, second,  rest :_*)(ClassTag.Any).castFrom[Array[Any], Arr[A]]

		@inline def prepended[A >: E](first :A, second :A, rest :A*) :Arr[A] =
			array.prepended(first, second, rest :_*)(ClassTag.Any).castFrom[Array[Any], Arr[A]]
...
case object RefArray extends RefArrayLikeFactory[RefArray] {
	/** Extension methods specific to [[net.noresttherein.sugar.collections.RefArray RefArray]] only.
	  ...
	  *
	  * @see [[net.noresttherein.sugar.collections.ArrayLike.RefArrayLikeExtension]] imported as
	...
	/** Wraps reference arrays in indexed sequences and unwraps all known mutable collections backed by arrays
	  ...
	  */
	object Wrapped {
		def apply[A](array :RefArray[A]) :mutable.IndexedSeq[A] =
			mutable.ArraySeq.make(array.castFrom[RefArray[A], Array[A]])

		def unapply[A](items :IterableOnce[A]) :Opt[RefArray[A]] = {
			val array = items match {
				case _ :immutable.Iterable[_] | _  :IndexedSeqOps[_, _, _] => //ignore immutable collections
					Lack
		...
		object Slice {
			def apply[A](array :RefArray[A], from :Int, until :Int) :collection.IndexedSeq[A] =
				ArraySliceSeq(array.castFrom[RefArray[A], Array[A]], from, until)
			...
			def unapply[A](items :IterableOnce[A]) :Opt[(RefArray[A], Int, Int)] = items match {
				...
				case _ :immutable.Iterable[_] | _ :IndexedSeqOps[_, _, _] => Lack //ignore immutable collections
	...
	private[collections] sealed trait RefArrayRank1Extensions extends Any with ArrayLike.extensions {
...
case object IRefArray extends RefArrayLikeFactory[IRefArray] with IterableFactory[IRefArray] {
	...
	/** Wraps and unwraps immutable `IndexedSeq` instances and other immutable collections backed by arrays
	  ...
	  */
	object Wrapped {
		def apply[A](array :IRefArray[A]) :IndexedSeq[A] =
			PassedArrayWrapper(array.castFrom[IRefArray[A], IArray[A]]) match {
				case Got(seq) => seq
				case _        => ArraySeq.unsafeWrapArray(array.castFrom[IRefArray[A], Array[A]])
			}
		...
		object Slice {
			def apply[A](array :IRefArray[A], from :Int, until :Int) :IndexedSeq[A] =
				PassedArrayWrapper.Slice(array.castFrom[IRefArray[A], IArray[A]], from, until) match {
					case Got(seq) => seq
					case _ => ArraySliceSeq.Immutable(array.castFrom[IRefArray[A], IArray[A]], from, until)
				}
			...
			def unapply[A](items :IterableOnce[A]) :Opt[(IRefArray[A], Int, Int)] = items match {
				...
				case seq :AbstractArraySlice[_] with immutable.Iterable[_] if seq.knownSize >= 0 =>
					Got(seq.unsafeArray.castFrom[Array[_], IRefArray[A]], seq.startIndex, seq.knownSize)

				case arr :AbstractArraySlice[A] with immutable.IndexedSeqOps[A, IndexedSeq, IndexedSeq[A]] @unchecked =>
					val array = arr.unsafeArray.castFrom[Array[_], IRefArray[A]]
					if (array.getClass == classOf[Array[AnyRef]])
						Got((array, arr.startIndex, arr.length))
					else
						Lack
				case _ =>
					Lack
...
//		case it :Wrapped(array)            => array
	...
	object Wrapped {
		@inline def apply[A](array :Array[A]) :collection.IndexedSeq[A] =
			ArraySliceSeq(array, 0, array.length) //not mutable.ArraySeq to be neither mutable nor immutable
		...
		object Slice {
			@inline def apply[A](array :Array[A], from :Int, until :Int) :collection.IndexedSeq[A] =
				ArraySliceSeq(array, from, until)
