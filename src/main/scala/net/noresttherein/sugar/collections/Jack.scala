		...
		override def inserted[U >: E](index :Int, elem :U) :Jack[U] = {
			...
			def insertIntoPrefix :Jack[U] =
				if (prefix.length < MaxChildren) //There is room in prefix, perform the simplest insertion
					...
					.length < MaxChildren) { //infix.length == 0; carry over last elements of prefix to suffix to make room
			...
			def insertIntoSuffix :Jack[U] = { //inserting within the suffix
