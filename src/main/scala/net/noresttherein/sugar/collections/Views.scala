...
private object Views {
	...
	@tailrec def zipEven[A, B](left :IterableOnce[A], right :IterableOnce[B]) :BaseView[(A, B)] =
		left match {
			case l :Iterable[A] => right match {
				case r :Iterable[B] => () => Iterators.zipEven(l.iterator, r.iterator)
				case _              => zipEven(l, TemporaryBuffer from right)
			}
	...
	@tailrec def zipMap[A, B, O](left :IterableOnce[A], right :IterableOnce[B])(f :(A, B) => O) :BaseView[O] =
		left match {
			case l :Iterable[A] => right match {
				case r :Iterable[A] => () => Iterators.zipMap(l.iterator, r.iterator)(f)
				case _              => zipMap(l, TemporaryBuffer from right)(f)
			}
	...
	@tailrec def zipMapEven[A, B, O](left :IterableOnce[A], right :IterableOnce[B])(f :(A, B) => O) :BaseView[O] =
		left match {
			case l :Iterable[A] => right match {
				case r :Iterable[A] => () => Iterators.zipMapEven(left.iterator, right.iterator)(f)
	...
	@tailrec def zipMapAll[A, B, O](left :IterableOnce[A], right :IterableOnce[B], leftElem :A, rightElem :B)
		...
		left match {
			case l :Iterable[A] => right match {
				case r :Iterable[A] => () => Iterators.zipMapAll(left.iterator, right.iterator, leftElem, rightElem)(f)
	...
	@tailrec def zipFlatMap[A, B, O](left :IterableOnce[A], right :IterableOnce[B])
		...
		left match {
			case l :Iterable[A] => right match {
				case r :Iterable[A] => () => Iterators.zipFlatMap(l.iterator, r.iterator)(f)
				case _              => zipFlatMap(l, TemporaryBuffer from right)(f)
			}
	...
	@tailrec def zipFlatMapEven[A, B, O](left :IterableOnce[A], right :IterableOnce[B])
		...
		left match {
			case l :Iterable[A] => right match {
				case r :Iterable[A] => () => Iterators.zipFlatMapEven(left.iterator, right.iterator)(f)
	...
	@tailrec def zip3[A, B, C](first :IterableOnce[A], second :IterableOnce[B], third :IterableOnce[C])
			:BaseView[(A, B, C)] =
		first match {
			case _1 :Iterable[A] => second match {
				case _2 :Iterable[B] => third match {
					case _3 :Iterable[C] => () => Iterators.zip3(_1.iterator, _2.iterator, _3.iterator)
	...
	@tailrec def zipEven3[A, B, C](first :IterableOnce[A], second :IterableOnce[B], third :IterableOnce[C])
		...
		first match {
			case _1 :Iterable[A] => second match {
				case _2 :Iterable[B] => third match {
					case _3 :Iterable[C] => () => Iterators.zipEven3(_1.iterator, _2.iterator, _3.iterator)
	...
	@tailrec def zipAll3[A, B, C](first :IterableOnce[A], second :IterableOnce[B], third :IterableOnce[C],
		...
		first match {
			case _1 :Iterable[A] => second match {
				case _2 :Iterable[B] => third match {
					case _3 :Iterable[C] =>
						() => Iterators.zipAll3(_1.iterator, _2.iterator, _3.iterator, firstElem, secondElem, thirdElem)
					case _               =>
	...
	@tailrec def zipTail[A](self :IterableOnce[A]) :BaseView[(A, A)] = self match {
		case items :Iterable[A] => () => Iterators.zipTail(self.iterator)
	...
	@tailrec def mapWith[A, B, C](self :IterableOnce[A], z :B, f :(A, B) => (C, B)) :BaseView[C] = self match {
		case items :Iterable[A] => () => Iterators.mapWith(items.iterator, z, f)
	...
	@tailrec def flatMapWith[A, B, C](self :IterableOnce[A], z :B, f :(A, B) => (IterableOnce[C], B)) :BaseView[C] =
		self match {
			case items :Iterable[A] => () => Iterators.flatMapWith(items.iterator, z, f)
			case _                  => flatMapWith(TemporaryBuffer from self, z, f)
		}

	@tailrec def mapWithIndex[A, B](self :IterableOnce[A], f :(A, Int) => B) :BaseView[B] =
		self match {
			case items :Iterable[A] => () => Iterators.mapWithIndex(items.iterator, f)
	...
	@tailrec def flatMapWithIndex[A, B](self :IterableOnce[A], f :(A, Int) => IterableOnce[B]) :BaseView[B] =
		self match {
			case items :Iterable[A] => () => Iterators.flatMapWithIndex(items.iterator, f)
	...
	@tailrec def collectWithIndex[A, B](self :IterableOnce[A], f :PartialFunction[(A, Int), B]) :BaseView[B] =
		self match {
			case items :Iterable[A] => () => Iterators.collectWithIndex(items.iterator, f)
	...
	@tailrec def mapWhile[A, B, C](self :IterableOnce[A], z :B, pred :B => Boolean, f :(B, A) => (B, C)) :BaseView[C] =
		self match {
			case items :Iterable[A] => () => Iterators.mapWhile(items.iterator, z, pred, f)
	...
	@tailrec def flatMapWhile[A, B, C](self :IterableOnce[A], z :B, pred :B => Boolean, f :(B, A) => (B, IterableOnce[C]))
			:BaseView[C] =
		self match {
			case items :Iterable[A] => () => Iterators.flatMapWhile(items.iterator, z, pred, f)
	...
	@tailrec def mapUntil[A, B, C](self :IterableOnce[A], z :B, f :(B, A) => (Boolean, B, C)) :BaseView[C] =
		self match {
			case items :Iterable[A] => () => Iterators.mapUntil(items.iterator, z, f)
	...
	@tailrec def flatMapUntil[A, B, C](self :IterableOnce[A], z :B, f :(B, A) => (Boolean, B, IterableOnce[C]))
			:BaseView[C] =
		self match {
			case items :Iterable[A] => () => Iterators.flatMapUntil(items.iterator, z, f)
	...
	@tailrec def mapSome[A, B, C](self :IterableOnce[A], z :B, f :(B, A) => Option[(B, C)]) :BaseView[C] =
		self match {
			case items :Iterable[A] => () => Iterators.mapSome(items.iterator, z, f)
	...
	@tailrec def flatMapSome[A, B, C](self :IterableOnce[A], z :B, f :(B, A) => Option[(B, IterableOnce[C])])
			:BaseView[C] =
		self match {
			case items :Iterable[A] => () => Iterators.flatMapSome(items.iterator, z, f)
	...
	@tailrec def mapPrefix[A, B, C](self :IterableOnce[A], z :B, f :(B, A) => Opt[(B, C)]) :BaseView[C] =
		self match {
			case items :Iterable[A] => () => Iterators.mapPrefix(items.iterator, z, f)
	...
	@tailrec def flatMapPrefix[A, B, C](self :IterableOnce[A], z :B, f :(B, A) => Opt[(B, IterableOnce[C])])
			:BaseView[C] =
		self match {
			case items :Iterable[A] => () => Iterators.flatMapPrefix(items.iterator, z, f)
	...
	@tailrec def filterWith[A, B](self :IterableOnce[A], z :B, pred :(A, B) => (Boolean, B), keep :Boolean) :BaseView[A] =
		self match {
			case items :Iterable[A] => () => Iterators.filterWith(items.iterator, z, pred, keep)
	...
	@tailrec def filterWithIndex[A](self :IterableOnce[A], pred :(A, Int) => Boolean, keep :Boolean) :BaseView[A] =
		self match {
			case items :Iterable[A] => () => Iterators.filterWithIndex(items.iterator, pred, keep)
	...
	@tailrec def keep[A](self :IterableOnce[A], pred :Int => Boolean) :View[A] =
		self match {
			...
			case items :Iterable[A] => (() => Iterators.keep(items.iterator, pred)) :BaseView[A]
