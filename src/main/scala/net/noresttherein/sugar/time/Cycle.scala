...
trait Cycle extends TimeProjector {
	...
	/** The current phase of this cycle at the particular moment in time.
	  * Uses `Time.`[[net.noresttherein.sugar.time.Time.Local Local]] as the reference.
	  */
...
//	def of(time :TimeLimes) :Phase = throw new UnsupportedOperationException(s"$this at $time")
	/** The current phase of this cycle on the particular date in the [[net.noresttherein.sugar.time.Time.Local local]]
	  * [[net.noresttherein.sugar.time.TimeZone time zone]].
	  */
...
object Cycle {
	...
	/** A cycle whose each individual [[net.noresttherein.sugar.time.Cycle.Phase Phase]] can be identified
	  * by a single `Long` value.
	  * @see [[net.noresttherein.sugar.time.Cycle.LongValuePhase]]
	  */
