...
class DateTime private[time] (val toJava :j.LocalDateTime) extends AnyVal with TimeProjection with Ordered[DateTime] {
	...
	def +(time :TimeFrame) :DateTime = time match {
		case t :TimeSpan => this + t
		case p :DateSpan => this + p
	...
	@inline def +(period :DateSpan) :DateTime = new DateTime(toJava plus period.period.toJava)
	@inline def -(period :DateSpan) :DateTime = new DateTime(toJava minus period.period.toJava)
	...
	@inline def +(time :TimeSpan) :DateTime = new DateTime(toJava plus time.toJava)
	@inline def -(time :TimeSpan) :DateTime = new DateTime(toJava minus time.toJava)
	...
	@inline def -(other :DateTime) :TimeInterval = new Duration(j.Duration.between(other.toJava, toJava))
