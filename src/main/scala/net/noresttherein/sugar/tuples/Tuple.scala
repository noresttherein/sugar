...
/** Generic base trait for tuples of varying number of elements. It is functionally very similar to ''shapeless' '' `HList`,
  ...
  *  - the 'Nil' type ([[net.noresttherein.sugar.tuples.Tuple.<>! <>]]) can be omitted from tuples of length &gt;=2 by
...
sealed trait Tuple extends Product {
	...
	/** Number of elements in this tuple. Same as [[net.noresttherein.sugar.tuples.Tuple.productArity productArity]]. */
	def length :Int
...
object Tuple {
	...
	/** Retrieves all elements from this tuple as a list with the type of their common upper bound. To match
	  * individual elements similarly to `List`s `::`, use [[net.noresttherein.sugar.tuples.Tuple.><$ ><]]
	...
	/** An empty tuple (a product of zero arity). Prepending elements with its
	  * [[net.noresttherein.sugar.tuples.Tuple.<>.>< ><]] operator results in creation of new tuples.
	  */
	final val <> : <> = new <>
	...
	/** An alias for a single element tuple `&lt;&gt; &gt;&lt; X`. New instances can be created either by the 
	  * companion object to this type, by the single-argument [[net.noresttherein.sugar.tuples.Tuple.apply[A](a:A)* Tuple(x)]],
	...
	/** An alias for a two element tuple `<> >< X >< Y` omitting the terminator type `<>` from the type signature,
	  ...
	  * [[net.noresttherein.sugar.tuples.Tuple.>:<$ >:<]]: `>:<(x, y)`, `Tuple(x, y)`, or an implicitly added extension
	...
	/** Patches any object implicitly adding the [[net.noresttherein.sugar.tuples.Tuple.Tuple2Constructor.\:\*]] method
	...
	/** Cartesian product of types in the tuple `P` and type `L`, being a type constructor for tuples with variable lengths.
	  * New instances can be created either by the [[net.noresttherein.sugar.tuples.Tuple$ Tuple]]'s object overloaded
	  * `apply` methods for fixed arities, or by recursively appending elements with [[net.noresttherein.sugar.tuples.Tuple!.>< &gt;&lt;]]
	  * to existing tuples. A natural start would be either the empty product [[net.noresttherein.sugar.tuples.Tuple.<>$ &lt;&gt;]]
	  * or a pair: [[net.noresttherein.sugar.tuples.Tuple.>:< >:<]].
	  ...
	  * @see [[net.noresttherein.sugar.tuples.Tuple.><$ companion object]]
	  */
	@SerialVersionUID(1L)
	final class ><[+P <: Tuple, +L] private[Tuple](elements :Array[Any], size :Int, owner :Boolean = true) extends Tuple {
		...
		/** Retrieves the `n`-th element of this tuple as a value of `LUB` type of all member types (that is the most
		  ...
		  * [[net.noresttherein.sugar.tuples.Tuple.><.apply[N](n:N)(implicit\ tpe:TupleAt[P><L,N,X]* Tuple.><(n)]]
		...
		/** Retrieves the `n`-th element of this tuple. The value of `n` must be statically known and encoded as a natural
		  * number type `Nat`. See [[net.noresttherein.sugar.tuples.Nat$ Nat]] for constants such as
		  * [[net.noresttherein.sugar.tuples.Nat$._1 _1]], [[net.noresttherein.tuples.Nat$._2]], ''etc.''.
		...
		/** Makes a projection of this tuple, selecting the elements at the given indices.
		  * @param indices a tuple with [[net.noresttherein.sugar.tuples.Nat Nat]] values,
...
//				nothing.asInstanceOf[TupleLUB[T >< L, U]]
		...
		/** An implicit witness providing the type `X` of `N`-th element in the tuple `T`.
		  ...
		  * @see [[net.noresttherein.sugar.tuples.Nat]]
		  */
