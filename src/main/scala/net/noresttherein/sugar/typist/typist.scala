...
package object typist {
	...
	/** A root of a phantom type hierarchy used to introduce subtyping relation to a single type `T`
	  ...
	  * scopes (or their companions), it is not always possible (as with definitions declared directly in a sugar)
	  ...
	  * (a companion object to some related class, a `syntax` sugar containing all implicits in the library,
	  * or a `imports` trait to be extended by application classes/sugar objects) - introducing
