...
package util {
	...
	/** Base class for serializable global values of type `S`.
	  * Expects a lazy expression resolving the value using a global path to which the object deserializes.
	  * This has an advantage over standard Scala object deserialization in that the concrete class of the deserialized
	  * object may be different from the class of this object, allowing for a greater decupling of implementation
	  * from the interface `S`.
	  */
	abstract class SerializableSingleton[+S](name :String, self: => S) extends Serializable { this :S =>
