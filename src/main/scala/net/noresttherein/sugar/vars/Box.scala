...
sealed trait Box[@specialized(SpecializedVars) T] extends InOut[T] with Serializable {
	/** Returns `!`[[net.noresttherein.sugar.vars.InOut.isDefined]].
	  * An empty box will throw a [[NoSuchElementException]] if the value is accessed by any method
	  * such as [[net.noresttherein.sugar.vars.InOut.value value]] or [[net.noresttherein.sugar.vars.InOut.get get]],
	  * but also [[net.noresttherein.sugar.vars.InOut.?= ?=]],
	  * [[net.noresttherein.sugar.vars.InOut.testAndSet testAndSet]] and others.
	  */
	@inline final def isEmpty :Boolean = !isDefined

	/** Returns `true` if the box currently holds a value. In case of atomic access,
	  * methods [[net.noresttherein.sugar.vars.InOut.value value]], [[net.noresttherein.sugar.vars.InOut.get]]
	  * and [[net.noresttherein.sugar.vars.Ref.apply apply]]`()` will not throw an exception.
	  * Equivalent to [[net.noresttherein.sugar.vars.Ref.isDefined isDefined]].
	  */
	...
	/** Replaces the contents of this box with the content of the given value wrapper
	  * (possibly a [[net.noresttherein.sugar.vars.Unsure Unsure]] or another [[net.noresttherein.sugar.vars.Box Box]]
	  * instance).
	  *
	  * If `content` is a [[net.noresttherein.sugar.vars.Val Val]], it simply assigns its value
	  * to this variable with `this.`[[net.noresttherein.sugar.vars.InOut.value_= value]]` = content.get`.
	  * This call can block awaiting the initialization of `content` by other threads, which possibly has not even begun
	  *  - for example for [[net.noresttherein.sugar.vars.Out Out]] or [[net.noresttherein.sugar.vars.SignalVal SignalVal]].
	  *    If you wish to assign the current value, use `this reset content.`[[net.noresttherein.sugar.vars.Ref.opt opt]]
	  *    or explicit conditional assignment instead.
	  *
	  * If `content` is a [[net.noresttherein.sugar.vars.Sure Sure]], the result is the same as above, while
	  * [[net.noresttherein.sugar.vars.Blank Blank]] results in the contents of this variable being immediately
	  * [[net.noresttherein.sugar.vars.Box.clear cleared]].
	  *
	  * If content is neither a `Val` nor a `Unsure`, it attempts to retrieve its value with either
	  * [[net.noresttherein.sugar.vars.Ref.opt opt]] or [[net.noresttherein.sugar.vars.Ref.asOption asOption]]
	...
	/** Replaces the contents of this box with the content of the given `Option`.
	  * It is a setter matching getter `this.`[[net.noresttherein.sugar.vars.Ref.? ?]] and is equivalent to both
	  * `this.`[[net.noresttherein.sugar.vars.Box.asOption_= asOption]]` = content`
	  * and `this `[[net.noresttherein.sugar.vars.InOut.:= :=]]` content.get` if `content` is `Some`,
	...
	/** Replaces the contents of this box with the content of the given `Option`.
	  * It is a setter matching getter `this.`[[net.noresttherein.sugar.vars.Ref.asOption asOption]]
