...
/** A monadic lazy value. While Scala `lazy val` is a better choice in general, and has a lesser memory footprint,
  ...
  *   1. They allow to check their initialization state (which in turn allows for example to discard `Delayed` and use
  *      the value directly, or choose a different method of initializing some other value).
  *   1. They provide monadic operations for constructing other `Delayed` instances.
  ...
  *      the cached value is `@transient` and becomes discarded when the `Delayed` instance is serialized,
  *      but the initializing block is always retained.
  *
  * @see [[net.noresttherein.sugar.vars.Lazy Lazy]]
  * @author Marcin Mościcki
  * @define Ref `Delayed`
  * @define ref delayed value
  */
trait Delayed[@specialized(SpecializedVars) +T] extends (() => T) with Val[T] with Serializable {
	/** An alias for [[net.noresttherein.sugar.vars.Delayed.isFinal isFinal]]/[[net.noresttherein.sugar.vars.Delayed.isDefinite isDefinite]]
	  * with a more intuitive name.
	  */
	@inline final def isEvaluated :Boolean = isDefinite

	/** True, if this value is already evaluated.
	  * @return [[net.noresttherein.sugar.vars.Delayed.isDefinite isDefinite]].
	  */
	...
	/** Returns `true`: all `Delayed` values can have only a single value and, once initialized,
	...
	/** Always returns `true`.
	  * @see [[net.noresttherein.sugar.vars.Delayed.isDefinite isDefinite]]
	  */
	...
	/** Returns `Some(`[[net.noresttherein.sugar.vars.Delayed.get get]]`)`. */
	override def toOption :Option[T] = Some(get)

	/** Returns `Some(`[[net.noresttherein.sugar.vars.Delayed.get get]]`)`,
	  * same as [[net.noresttherein.sugar.vars.Delayed.toOption toOption]].
	  */
	...
	/** Returns [[net.noresttherein.sugar.vars.Sure Sure]]`(`[[net.noresttherein.sugar.vars.Delayed.get get]]`)`. */
	...
	/** Creates a new $Ref`[O]` instance with the same characteristics as this instance, evaluated to the application
	  ...
	  * accessed. Created `Delayed[O]` will likewise use the `synchronized` block in that case and `f` will be evaluated
	  * at most once.
	  */
	override def map[O](f :T => O) :Delayed[O] =
		new MappedVal[T, O](this, f) with Delayed[O]
	...
	/** Creates a new $Ref`[O]` initialized with the expression `f(this.value).value`. If this instance is already
	  ...
	  */
	def flatMap[O](f :T => Delayed[O]) :Delayed[O] =
		new FlatMappedVal[T, O](this, f) with Delayed[O]

	override def mkString :String = mkString("Delayed")
...
case object Delayed {
	...
	/** Creates a wrapper over a lazily initialized value roughly equivalent to a built-in `lazy val`.
	  ...
	  *   - monadic [[net.noresttherein.sugar.vars.Delayed.map map]]
	  *     and [[net.noresttherein.sugar.vars.Delayed.flatMap flatMap]] operations;
	  ...
	  */
	@inline def apply[@specialized(SpecializedVars) T](init : => T) :Delayed[T] = from(() => init)

	/** Same as [[net.noresttherein.sugar.vars.Delayed.apply apply]], but accepts a `Function0`,
	  * rather a by-name parameter.
	  */
	def from[@specialized(SpecializedVars) T](init: () => T) :Delayed[T] =
		new SyncDelayedVal[T] match {
			case ref if ref.getClass == classOf[SyncDelayedVal[Any]] => new SyncDelayedRef(init)
			case spec => spec.init(init); spec
		}

	/** Unlike the default `Delayed(_)` and built in `lazy val`s, this lazy value does not use a `synchronized` block,
	  ...
	  */
	@inline def idempotent[@specialized(SpecializedVars) T](idempotent : => T) :Delayed[T] = Pure(idempotent)

	/** Similar to [[net.noresttherein.sugar.vars.Delayed.idempotent idempotent]] lazy value, but on serialization,
	  ...
	  */
	@inline def transient[@specialized(SpecializedVars) T](idempotent :Eval[T]) :Delayed[T] = Transient[T](idempotent)


	/** A wrapper over a computed value adapting it to the `Delayed` type. */
	def eager[@specialized(SpecializedVars) T](value :T) :Delayed[T] = new Eager(value)


	implicit def unboxLazy[T](l :Delayed[T]) :T = l.const

	implicit def lazyOrdering[V[X] <: Delayed[X], T :Ordering] :Ordering[V[T]] = new RefOrdering[V, T]
	implicit def lazyNumeric[T :Numeric]       :Numeric[Delayed[T]]    = new RefNumeric[Delayed, T] with LazyTypeClass[T]
	implicit def lazyIntegral[T :Integral]     :Integral[Delayed[T]]   = new RefIntegral[Delayed, T] with LazyTypeClass[T]
	implicit def lazyFractional[T :Fractional] :Fractional[Delayed[T]] = new RefFractional[Delayed, T] with LazyTypeClass[T]
	
	private trait LazyTypeClass[T] extends RefNumeric[Delayed, T] {
		override def fromInt(x :Int) :Delayed[T] = Delayed(underlying.fromInt(x))
		protected override def fmap(x :Delayed[T], y :Delayed[T])(op :(T, T) => T) :Delayed[T] =
			for (xv <- x; yv <- y) yield op(xv, yv)

		protected override def map(x :Delayed[T])(f :T => T) :Delayed[T] = x.map(f)
		protected override def apply(x :T) :Delayed[T] = Delayed.eager(x)
	}
	...
	private final class Eager[+T](override val get :T) extends Const[T] with Delayed[T] {
...
/** `Delayed` implementation equivalent in semantics to Scala's `lazy val`.
  ...
  */
@SerialVersionUID(Ver)
private class DelayedVal[+T] extends Delayed[T] {
	...
	override def value :T = {
		...
		else noSuch_!("Uninitialized Delayed")
	}
	override def get :T = {
		var res = evaluated
		if (res == undefined)
	...
	override def map[O](f :T => O) :Delayed[O] = {
		val v = evaluated
		if (v != undefined)
			Delayed.eager(f(v.asInstanceOf[T]))
		else synchronized {
			val init = initializer
			if (init == null) {
...
@SerialVersionUID(Ver)
private class SyncDelayedVal[@specialized(SpecializedVars) +T] extends DelayedVal[T] with Delayed[T] {
//	override def value :T
//	override def get :T
...
//	override def isSpecialized = true
	...
	private def writeReplace :AnyRef = Delayed.eager(get)
}



@SerialVersionUID(Ver)
private[sugar] class SyncDelayedRef[+T](private[this] var initializer :() => T) extends Delayed[T] {
	...
	final override def value :T = evaluated match {
		case `undefined` => noSuch_!("Uninitialized Delayed")
...
/** A mix-in trait for application classes with a full lazy field implementation, in particular for various lazy proxies.
  * It closely resembles [[net.noresttherein.sugar.vars.Delayed Delayed]], but does not extend any trait
  ...
  * @see [[net.noresttherein.sugar.vars.AbstractDelayed AbstractLazy]]
  * @see [[net.noresttherein.sugar.vars.AbstractPure AbstractPure]]
  */ //consider: dropping specialization.
sealed trait DelayedOps[@specialized(SpecializedVars) +T] {
...
/** A trait with a lazy field, initialized under this object's monitor.
  ...
  * The initialized field value can be accessed using [[net.noresttherein.sugar.vars.DelayedOps.definite definite]]
  * property, and [[net.noresttherein.sugar.vars.DelayedOps.isDefinite isDefinite]] returning `false` means the field
...
/** A full [[net.noresttherein.sugar.vars.Pure Pure]]-like lazy value implementation as a mix-in trait
  ...
  * For this reason all methods are protected, with subclasses having full control over what API they want to expose.
  */
trait AbstractPure[@specialized(SpecializedVars) +T] extends DelayedOps[T]
