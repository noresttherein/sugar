...
/** A `@specialized` (for all value types with the exception of `Unit`) version of [[scala.Option]]
  ...
  * and [[net.noresttherein.sugar.vars.Blank Blank]] (equivalent of `None`).
...
sealed trait Unsure[@specialized(SpecializedVars) +T] extends Ref[T] with IterableOnce[T] with Product with Serializable {
	/** Tests if this `Unsure` does not contain a value (is equal to [[net.noresttherein.sugar.vars.Blank Blank]]). */
	...
	/** Returns `this` ''iff'' this value is [[net.noresttherein.sugar.vars.Blank Blank]],
	...
	/** Returns a new `Unsure` containing this value if it is not empty and its value satisfies the given predicate,
	  * or [[net.noresttherein.sugar.vars.Blank Blank]] otherwise.
	  */
...
object Unsure {
	...
	/** Returns [[net.noresttherein.sugar.vars.Blank Blank]] in a manner consistent with the collections hierarchy. */
	...
	/** When a given condition is true, evaluates the `value` argument and returns
	  ...
	  * and [[net.noresttherein.sugar.vars.Blank Blank]] is returned.
	  */
...
//
	...
	object implicits {
		...
		/** A nomen omen optional implicit conversion of an `Opt[A]` to a `Unsure[A]`.
		  * @see [[net.noresttherein.sugar.vars.Unsure.OptionToUnsureConverter]]
		  */ //consider: placing this also in vars.extensions (or vars.implicits/vars.imports)
	...
	/** Importing the contents of this object replace all usage of [[Option]]/[[Some]]/[[None]] in the scope with
	  * [[net.noresttherein.sugar.vars.Unsure Unsure]]/[[net.noresttherein.sugar.vars.Sure Sure]]/[[net.noresttherein.sugar.vars.Blank Blank]].
...
/** An empty (''blank'') [[net.noresttherein.sugar.vars.Unsure Unsure]] instance, a counterpart of [[scala.None]]. */
@SerialVersionUID(1L)
