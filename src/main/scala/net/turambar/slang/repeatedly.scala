...
import net.turambar.slang.repeatedly.repeatingCollections.{IteratePartialFunction, IterateSome}
...
object repeatedly {
	...
	/** A complement of `C.iterate` provided by collection companion objects, which creates a collection `C`
	  ...
	  * @see [[net.turambar.slang.repeatedly.repeatingCollections.IteratePartialFunction#apply[X](X)(PartialFunction[X, X])(CanBuildFrom[_, X, C])]]
	  */
	...
	/** A complement of `C.iterate` provided by collection companion objects, which creates a collection `C`
	  ...
	  * @tparam C return collection type
	  * @return a collection containing the sequence starting with `start` and resulting from recursively applying `next` to itself.
	  * @see [[net.turambar.slang.repeatedly.repeatingCollections.IterateSome#apply[X](X)(X=>Optioin[X])(CanBuildFrom[_, X, C])*]]
	  */
