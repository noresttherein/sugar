...
object IterableOnceExtensionSpec extends Properties("IterableOnceExtension") {
	...
	private def foldingProperty(prop :(Int, Iterable[Int]) => Prop) :Prop =
		forAll { (start :Int, numbers :List[Int]) => prop(start, numbers) :| "List" } &&
			...
			forAll { (start :Int, numbers :RefArraySlice[Int]) => prop(start, numbers) :| "ArraySlice" } &&
	...
	property("foreachInRange") = iterableProperty { list :Iterable[Int] =>
		forAll { (from :Int, until :Int) =>
			var result, expect = 0
			list.foreachInRange(from, until)(result += _)
	...
	property("forPrefix") = iterableProperty { list :Iterable[Int] =>
		forAll { len :Int =>
			var result, expect = 0
			list.forPrefix(len)(result += _)
			list.take(len).foreach(expect += _)
			result ?= expect
		}
	}
	property("forSuffix") = iterableProperty { list :Iterable[Int] =>
		forAll { len :Int =>
			var result, expect = 0
			list.forSuffix(len)(result += _)
			list.takeRight(len).foreach(expect += _)
			result ?= expect
		}
	}
	...
	property("forallWith") = iterableProperty { list :Iterable[Int] =>
		val zipped = list.zip(Iterator.iterate(3)(_ + 1))
		(list.forallWith(3)((e, acc) => (acc + 1, e % acc > 0)) ?=
			zipped.forall { case (e, acc) => e % acc > 0 }) lbl zipped.toString
	}
	property("forallSome") = iterableProperty { list :Iterable[Int] =>
		val zipped = list.zip(Iterator.iterate(3)(_ + 1))
		(list.forallSome(3)((e, acc) => Option.when(e % acc > 0)(acc + 1)) ?=
			zipped.forall { case (e, acc) => e % acc > 0 }) lbl zipped.toString
	}
