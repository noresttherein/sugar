...
object IntRatioSpec extends Properties("IntRatio") {
	...
	val Force = MatchPattern { x :Int => Yes(if (x == Int.MinValue) -Int.MaxValue else x) }
	val InRange = MatchPattern { x :Int => if (x == Int.MinValue) No else Yes(x) }



	private def constructorProp(f :(Int, Int) => IntRatio) :Prop = forAll { (n :Int, d :Int) =>
		if (d == 0)
	...
	property("apply(Int, Int)") = constructorProp(IntRatio.apply)
	property("%/") = constructorProp(_ %/ _)
	property("apply(Long, Long)") = constructorProp((n, d) => IntRatio(n.toLong, d.toLong))
	...
	private def additionProp(rational :(IntRatio, IntRatio) => IntRatio, s :String) :Prop =
		forAll { (x1 :Int, x2 :Int, x3 :Int, x4 :Int) =>
			...
			nonZeros match {
				...
				case Seq(Force(x)) =>
					(IntRatio(x) =? rational(x, 0) label s"x =? x $s 0") && (IntRatio.unit(x) =? rational(1 %/ x, 0) label s"1 %/ x =? 1 %/ x $s 0")
				case Seq(Force(x), Force(y)) =>
					val prop =
						if (x + y == x.toLong + y && x + y != Int.MinValue)
							IntRatio(x + y) =? rational(x, y) label s"x=$x, y=$y: (x + y) %/ 1 =? x %/ 1 $s y %/ 1"
						else
							Prop.passed
					...
					def props(x :Int, y :Int, z :Int) :Prop = {
						val prop =
							if (x * y != x.toLong * y || x * y == Int.MinValue ||
                                ...
                                IntRatio(y + z * x, x * y) =? rational(1 %/ x, z %/ y) label
	                                s"x=$x, y=$y, z=$z: (y + z * x) %/ (x * y) =? 1 %/ x $s z %/ y"
						if (x * y + z != x.toLong * y + z || x * y + z == Int.MinValue)
							prop
						else
							prop && (IntRatio(x * y + z, y) =? rational(x, z %/ y) label s"x=$x, y=$y, z=$z: (x * y + z) %/ y =? x $s z %/ y")
					}
				...
				case _ =>
					def props4(w :Int, x :Int, y :Int, z :Int) :Prop =
						if (w * x + y * z != w.toLong * x + y.toLong * z || w * x + y * z == Int.MinValue ||
							...
							IntRatio(w * x + y * z, x * y) =? rational(w %/ y, z %/ x) label
	...
	private def subtractionProp(rational :(IntRatio, IntRatio) => IntRatio, s :String) :Prop =
		forAll { (x1 :Int, x2 :Int, x3 :Int, x4 :Int) =>
			val nums = Seq(x1, x2, x3, x4)
			val nonZeros = nums.filterNot(_ == 0)
			nonZeros match {
				...
				case Seq(Force(x)) =>
					(IntRatio(x) =? rational(x, 0) label s"x =? x $s 0") && (IntRatio.unit(x) =? rational(1 %/ x, 0) label s"1 %/ x =? 1 %/ x $s 0") &&
					(IntRatio(-x) =? rational(0, x) label s"-x =? 0 $s x") && (IntRatio.unit(-x) =? rational(0, 1 %/ x) label s"-1 %/ x =? 0 $s 1 %/ x")
				case Seq(Force(x), Force(y)) => all(allOf(
					IntRatio(x - y) =? rational(x, y) label s"x=$x, y=$y: (x - y) %/ 1 =? x %/ 1 $s y %/ 1" providing
						(x - y == x.toLong - y && x - y != Int.MinValue),
					((x * y - 1) %/ y =? rational(x, 1 %/ y) label s"x=$x, y=$y: (x*y - 1) %/ y =? x $s 1 %/ y") &&
				...
				case Seq(Force(x), Force(y), Force(z)) =>
					def props(x :Int, y :Int, z :Int) :Prop = all(allOf(
						IntRatio(y - z * x, x * y) =? rational(1 %/ x, z %/ y) label
							...
							(x * y != x.toLong * y || x * y == Int.MinValue ||
								y - z * x != y + z * x.toLong || y - z * x == Int.MinValue),
						IntRatio(x * y - z, y) =? rational(x, z %/ y) label s"x=$x, y=$y, z=$z: (x * y - z) %/ y =? x $s z %/ y" unless
	...
	private def multiplicationProp(rational :(IntRatio, IntRatio) => IntRatio, s :String) :Prop =
		forAll { (x1 :Int, x2 :Int, x3 :Int, x4 :Int) =>
			val nums = Seq(x1, x2, x3, x4)
			val nonZeros = nums.filterNot(_ == 0)
			nonZeros match {
				...
				case Seq(Force(x)) =>
					(IntRatio(x) =? rational(x, One) label s"x=$x: x $s 1 =? x") &&
						(1 %/ x =? rational(One, 1 %/ x) label s"x=$x: 1 %/ x =? 1 $s 1 %/ x")
				case Seq(Force(x), Force(y)) =>
					val prop = (x %/ y =? rational(x, 1 %/ y)) :| s"x=$x, y=$y: x %/ y =? x $s 1 %/ y"
					if (x * y == x.toLong * y.toLong)
						prop && (IntRatio(x * y) =? rational(x, y)) :| s"x=$x, y=$y: x*y/1 =? x $s y" &&
							(IntRatio.unit(x * y) =? rational(1 %/ y, 1 %/ x)) :| s"x=$x, y=$y: 1/x*y =? 1 %/ y $s 1 %/ x"
					else
						prop
				case Seq(Force(x), Force(y), Force(z)) =>
					def props(x :Int, y :Int, z :Int) :Prop =
	...
	private def divisionProp(rational :(IntRatio, IntRatio) => IntRatio, s :String) :Prop =
		forAll { (x1 :Int, x2 :Int, x3 :Int, x4 :Int) =>
			val nums = Seq(x1, x2, x3, x4)
			val nonZeros = nums.filterNot(_ == 0)
			nonZeros match {
				...
				case Seq(Force(x)) =>
					(IntRatio(x) =? rational(x, One) label s"x=$x: x/1 $s 1 ?= x/1") &&
						(1 %/ x =? rational(One, x) label s"x=$x: 1 $s x = 1/x")
				case Seq(Force(x), Force(y)) =>
					val prop = (x %/ y =? rational(x, y)) :| s"x=$x, y=$y: x %/ y =? x/1 $s y/1"
					if (x * y == x.toLong * y.toLong)
						prop && (IntRatio(x * y) =? rational(x, 1 %/ y)) :| s"x=$x, y=$y: x*y/1 =? x $s 1/y" &&
							(IntRatio(x * y) =? rational(y, 1 %/ x)) :| s"x=$x, y=$y: x*y/1 =? y $s 1/x" &&
